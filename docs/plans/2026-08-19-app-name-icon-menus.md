...
| 9 | 창이 보인 **직후**(수 ms)에 보내도 작업 표시줄이 갱신된다 | **전제 자체가 틀렸다 — 창이 보인 뒤에는 무엇을 해도 정공법이 아니다.** 작업 표시줄은 버튼을 만드는 순간 아이콘을 읽으므로 **그보다 앞서** 붙여야 한다(winit이 창 클래스를 비워 두어 그 자리가 빈다). 최종 구현은 창이 보이기 전 1회로 바뀌었고, 「지연을 얼마로 잡을까」라는 물음 자체가 사라졌다 | ❌ 기각 (T2 재수정) |
...
- **T2 재수정 후 재검증 (사용자 지적 반영)** — 반복 전송을 폐기하고 「창이 보이기 전 1회 + 클래스 아이콘」으로 바꾼 뒤 `cargo test` 858/858 · clippy 0 · release 캡처 대조를 다시 통과했다.
...
> **✅ 2026-08-20 사용자 확인 완료 — 아래 전 항목 정상.** 자동 캡처로는 판정할 수 없어(egui 앱이라 accesskit이 꺼져 있다) 사람이 직접 눌러 확인한 축이다.
...
- **T2를 다시 고쳤다 — 반복 전송은 우회였고 원인 진단이 틀렸다(사용자 지적, 2026-08-19)**. 처음 구현은 「창이 보인 뒤 0.3~3.1초에 걸쳐 여덟 번 재전송」이었는데, 이는 정상적인 방법이 아니다. 위키의 Rust 선례(`20_projects/personal/maid/feat-app-shell`)가 정답을 갖고 있었다 — **보통은 창 클래스를 등록할 때 리소스 아이콘을 넣고**(gpui가 그렇게 한다) 그러면 실행 파일·창 좌상단·작업 표시줄이 한 번에 같은 그림이 된다. **winit 0.30은 그것을 하지 않는다**(`register_window_class`가 `hIcon: 0`으로 등록하고 `WM_SETICON`도 ICON_SMALL만 세운다). 그래서 작업 표시줄이 버튼을 만드는 순간 이 창에는 아이콘이 없고, eframe이 뒤늦게 붙여도 반영되지 않는다.
  - **최종 해결**: `ShellHost`가 HWND를 얻는 자리(첫 프레임 이전 = 창이 보이기 전)에서 **창 아이콘과 창 클래스 아이콘을 함께 한 번** 세운다. 반복·지연·NULL 비우기가 모두 사라졌고 코드가 79줄 줄었다. release에서 캡처 대조로 확인했다(실행 중일 때만 MOA 아이콘이 나타나고 종료하면 사라진다).
  - **중간 진단이 두 번 빗나간 이유**: ① 「같은 핸들 재설정이라 갱신이 감지되지 않는다」는 결론은 *창이 보인 뒤*에 보내는 전제에서만 성립하는 관측이었다. ② 이른 시점 설정이 실패했다고 판정한 것은 **측정 오류**였다 — PowerShell의 `MainWindowHandle`이 winit의 메시지 전용 창(`Winit Thread Event Target`, 16×16)을 잡았다. 실제 앱 창(`Window Class`)은 t=2초·`vis=False` 시점에 이미 아이콘이 붙어 있었다. **창을 여럿 만드는 앱에서는 어느 창을 쟀는지부터 확인해야 한다.**
