...
## Phase G — PRD 요구 재검증 (G-1, 2026-08-17)
F-7(Opus, 2라운드)이 수행한 PRD 전수 대조를 재사용했다(정상 경로 — 과부하 대체·체크리스트 대체 없음).

| PRD ID | 우선순위 | 충족 | 근거 |
|--------|---------|------|------|
| FR-6 (주소창·탐색 + **열기 실패 표시 규칙**) | Must | ✅ (기계 검증분) / ⏳ HUMAN-VERIFY (화면) | T1·T2 구현 + T7 문면. 세 갈래 사유 `ui/panel.rs`의 `block_list`·`ListBlock`, 문구 `i18n/mod.rs:225-232`, `NotFound`만 현 위치 — 시험 `열어_본_결과를_닿았는가로_갈라_올린다`·`찾을_수_없는_폴더는_현_위치를_지킨다`·`네트워크가_끊기면_그_경로로_옮기고_목록_자리에_사유를_적는다` 통과. PRD가 지정한 검증법이 "단위테스트 + 화면은 HUMAN-VERIFY"라 화면 몫은 사용자 확인 |
| FR-9 (폴더 트리 + **연결 끊김 X 배지**) | Should | ✅ (기계 검증분) / ⏳ HUMAN-VERIFY (화면) | T5 배지 + T7 문면. `draw_offline_badge`, 네트워크 한정(`app/drives.rs`의 `row.network` 게이트), 시작 1회(`spawn_scan`+`poll_drives`) + 열어 볼 때 갱신(T6 4단 홉) — 시험 6건 통과. PRD 검증법이 `HUMAN-VERIFY`라 화면 몫은 사용자 확인 |
| 그 밖의 active Must/Should FR | — | 대조 제외 | `## PRD Coverage`가 `이번 범위 외 (기구현)`로 선언 — 기존 제품의 후속 plan이라 이 회차가 건드리지 않았다 |

**Must 충족률 100%** (커버 대상 Must는 FR-6 하나이며 기계 검증분 충족). **G-2 갭 처리 없음** — Must·Should 미충족 0건이라 재루프를 돌지 않았다.
...
| 드라이브 줄을 워커가 만들면 **첫 프레임에 드라이브가 없다** | 트리가 잠깐 즐겨찾기만 보인다 | 워커가 **목록과 접근 판정을 나눠 두 번 보내고 보낼 때마다 화면을 깨운다**(T4 · 2026-08-17 F-7 정정 — 처음에는 마지막에 한 번만 깨워, 입력이 없으면 프레임을 돌리지 않는 이 앱에서 목록 선행 도착이 헛일이 됐다) — 무거운 접근 판정(끊긴 드라이브 실측 2.8초)을 기다리지 않고 목록이 먼저 뜬다. 빈 사이에 "읽는 중" 표시는 두지 않는다(깜빡임이 더 눈에 띈다) |
...
## Risks 실현 검토 (F-5, 2026-08-17)
| 위험 | 실현됐는가 | 완화책이 들었는가 |
|---|---|---|
| `LayoutOutcome`→`ui::app` 홉 미강제 | 실현 안 됨(그 줄을 넣었다) | 완화책 자체를 **정정**했다 — 그 홉은 시험으로 덮을 수 없어(`ExplorerApp`은 실 HWND 필요) 앞뒤 세 층을 덮고 사이의 3줄은 리뷰가 지킨다는 사실을 코드·plan에 명시 |
| 드라이브 줄을 워커가 만들면 첫 프레임에 비어 있다 | **실현**(설계대로) | 들었다 — 조회를 목록·판정으로 나눠 목록이 먼저 도착한다. 무거운 접근 판정(2.8초)을 기다리지 않는다 |
| 시험 헬퍼가 접근 판정 포함 함수를 쓰면 `cargo test`가 느려진다 | **실현, 그리고 예상보다 컸다** | 들었다 — 시험은 `list_drives`(판정 없음)만 쓴다. 다만 **예상 밖의 원인**이 하나 더 있었다: 하네스가 프레임마다 조회해 스위트가 10분을 넘겼고(`IconCache::new()` + 셸 잠금), `OnceLock`으로 1회 조회로 묶어 10.6초로 회복했다 |
| 워커의 COM 초기화 누락 | 실현 안 됨 | 들었다 — `fs::thumbnail` 선례대로 진입에서 열고 종료에서 닫는다 |
| 네트워크 오류 코드 집합이 좁아 일부 실패가 샌다 | 실현 여부 미확인(목록 밖 코드를 만나지 못했다) | 들었다 — 그리고 **배지 판정을 그 깃발에서 떼어** 위험 자체를 줄였다(코드가 빠져도 배지는 붙는다). 문구만 덜 구체적이 된다 |
...
**F-8 완료 (2026-08-17)** — 아래 두 행은 배지가 9px 남짓이라 기준 스크린샷에서 정밀 측정이 불가능해 `⏳ 미확인`으로 인계했다. **사용자가 화면으로 확인해 통과 판정했다** — 코드에 반영된 값(`BADGE_STROKE_RATIO = 0.15`·`BADGE_MARK_RATIO = 0.5`)이 그대로 유효하다:
- ✅ X 선 굵기 (배지 지름의 0.15배)
- ✅ X 선 길이 (배지 지름의 0.5배)

**이로써 `⏳ 미확인` 잔존 0 — F-8 게이트 통과.**
...
| X 배지 | X 선 길이 | X가 차지하는 **사각 영역의 한 변**이 배지 지름의 **0.5배**(중심에서 양쪽 0.25배씩) — 대각선 하나의 실제 길이는 그 √2배다 | ⚠ 어림값 — 위와 같다. 기준 스크린샷에서는 X가 원 안쪽에 여백을 두고 들어 있는 것까지만 읽힌다 |
...
- [x] T2. 목록 자리에 열기 실패 사유를 적는다
  ...
  - **Acceptance**:
    - Given 네트워크가 끊긴 드라이브, When 그 경로로 이동, Then 주소창·트리가 **그 경로**를 가리키고 목록이 비며(올라갈 곳이 있으면 첫 줄 `..`만 남는다 — 드라이브 뿌리에는 그 줄도 없다) 그 자리에 `네트워크 드라이브에 연결할 수 없어 내용을 표시할 수 없습니다`가 뜬다
...
- [x] T6. 열어 본 결과로 드라이브 상태를 갱신한다
  ...
  - **Edge Cases**:
    - **`LayoutOutcome`→`ui::app` 홉은 컴파일러도 시험도 미치지 않는다**(전제 6) — 이 한 줄을 빠뜨리면 조용히 갱신이 끊기는데, `ExplorerApp`은 실 HWND(`CreationContext`)가 있어야 만들어져 프레임을 돌리는 시험을 세울 수 없다. 사슬을 세 층으로 나눠 각각 덮고(값을 세우는 곳·모으는 곳·반영 규칙) **그 사이의 대입 줄은 리뷰가 지킨다** — 그 사실을 코드 주석에도 적는다(2026-08-17 정정: 종전 문면은 없는 시험 보호를 주장했다)
...
- [x] T7. PRD·README 문면을 맞추고 위키에 큐를 남긴다
  ...
  - **Acceptance**:
    ...
    - 위키 vault 루트 `pending.md`에 1줄 — `feat-navigation.md`의 열거 실패 서술이 바뀌었다는 것. **태그는 `[K-DRIFT]`를 쓴다**(2026-08-17 정정: 원안은 `[PROJECT-FACT]`였으나 이것은 위키 페이지가 코드와 어긋났다는 드리프트 보고이므로 그 태그가 정확하다 — 크로스 세션 지식 자체는 별도 `[PROJECT-FACT]` 3줄로 큐잉했다)
  - **Files**:
...
## Phase Ledger
Phase F 통과 (HEAD eedf473 + m3 정정) · Phase G 통과 (Must 100%) · **F-8 통과 (2026-08-17 사용자 화면 확인 — ⏳ 미확인 2행 모두 ✅)**
...
## Progress Log
- T7 + Phase F (커밋 48a5909 이후): PRD FR-6·FR-9 문면과 README 세 곳을 코드에 맞췄고 위키에 `[K-DRIFT]`·`[PROJECT-FACT]` 4줄을 큐잉했다. Deferred 대장은 반영 1건 삭제·신규 3건 등재.
  - F-7 지적 반영: **워커가 두 번 보내는데 화면을 한 번만 깨우던 것**을 고쳤다(보낼 때마다 `request_repaint` — 이 앱은 입력이 없으면 프레임을 돌리지 않아 목록 선행 도착이 헛일이었다). plan 한 곳에 남아 있던 "프레임 시험으로 덮는다" 주장과 X 선 길이 서술(0.5는 축 방향 폭이고 대각선은 √2배)도 정정.
- T5-T6 (커밋 5950a80, 12564b6): 배지를 그리고, 열어 본 결과를 4단 홉으로 앱까지 올려 반영한다. 시험 808건.
...
## Next Steps
- 권장 다음 액션: **없음 — 이 plan은 완료됐다.** F-8 화면 확인까지 통과했고(2026-08-17), master 병합·push도 사용자 승인으로 마쳤다.
- Suggested skills: 공식 `/code-review`, 공식 `/security-review`
- 위키 갱신: 이번 회차의 `[DECISION]`·`[PROJECT-FACT]` 큐는 F-6.5 자동 소비로 처리했다(결과는 최종 보고). `[K-DRIFT]`(feat-navigation 드리프트)는 위키 세션의 ingest 몫으로 큐에 남는다.
