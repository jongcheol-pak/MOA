...
- **Edge Cases**: 빈 폴더(행 0) / 접근 거부(`EnumOutcome::AccessDenied` → 사유 표시. **T6에서 상위 결정으로 변경** — pending-커밋 모델을 도입하면서 실패 시 목록을 비우지 않고 현 위치를 유지한다)  / 260자 초과·유니코드 파일명(NFR-5) / 열거 중 폴더 이동 → 세대 불일치 결과 폐기 / 정렬 중 항목 0 / `ImageList_GetIcon` 실패(null HICON) → 아이콘 생략 / Shift 범위 선택 중 목록이 갱신됨 → 선택 인덱스 클램프
...
### 구현 중 추가된 항목 (사전 승인 목록에 없었으나 비파괴로 판단해 진행)

리뷰 지적을 해소하며 필요해진 변경이다. 셋 다 **기존 심볼의 동작·시그니처를 바꾸지 않아** 현행 Win32 앱에 영향이 없다(F-7 리뷰가 "저촉 아님"으로 확인):

- `src/panel/tabs.rs`에 `TabsModel::close(index)` **신규 추가**(+테스트 4개) — 기존 `close_active`와 그 Win32 호출자는 무변경. 배경 탭을 닫을 때 활성 탭이 튀는 문제(T5 리뷰 BLOCKER) 해소에 필요했다
- `src/fs/shell_menu.rs`에 `debug_assert!` 추가 — 유일한 호출부가 이미 분기해 도달 불가라 실질 동작 영향 0
- `src/panel/file_list.rs`의 `compare_entries` 주석 보강 (가시성 승격은 원래 승인 항목)
...
- **T5~T7 완료** (커밋 `d226ba0`·`b8b64bb`·`b18130a`): 탭 스트립(T5) → 패널 조립(T6) → 자유 분할(T7). 리뷰가 T5에서 BLOCKER 1건(배경 탭 닫기), T7에서 BLOCKER/MAJOR 2건(분할 판정 영역·닫은 뒤 활성 선택)을 잡아 모두 수정했다.
  - **T6는 순수 재배치**였지만 그 과정에서 T3이 세운 불변식이 깨졌다 — 셸 메뉴 모달 호출이 그리기 클로저 **안**으로 들어갔고, Phase F의 plan-completion-reviewer가 이를 잡았다. `MenuRequest`를 값으로 반환해 `ExplorerApp::ui`가 클로저 밖에서 띄우도록 되돌렸다. **재배치 리팩토링은 "로직이 같은가"뿐 아니라 "호출 위치 제약이 유지되는가"까지 봐야 한다.**
  - **Phase F 정정 사항**: 테스트 수는 **101**(lib 97 + layout_flow 2 + watcher 2)이다 — 중간 커밋 메시지의 "97+2+2+2"는 lib에 포함된 splitter 2개를 이중 계상한 것. 고아 심볼 2개(`FileListView::clear`·`IconTextures::created`)는 제거했고, deferred 대장의 shell_menu 항목은 종결로 옮겼다.
