...
- **[SUGGEST] 전역 공유 자원 묶기** — `PanelState::show`가 `icons`·`textures`·`shell` 3개를 개별 인자로 받는다. T7에서 패널이 여러 개가 되면 호출부마다 같은 인자를 반복 전달하게 되므로, `SharedResources<'a>` 같은 묶음으로 감싸면 이후 공유 자원이 늘어도 시그니처가 안 바뀐다 (T6 quality 리뷰 S1 — **T7에서 반영 검토**)
...
- T1~T6 완료 (전부 Type C — V-1~V-8 수행, spec·quality 리뷰 통과)
