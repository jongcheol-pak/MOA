...
## Phase Ledger
- Phase F 통과 (HEAD 0ff96e5 기준 — F-1~F-7 전부 통과, F-7 지적 BLOCKER 0 / MAJOR 0 / MINOR 4는 문서 정정·등재로 해소)

## Retry Ledger

## Progress Log
- T1-T2 완료 (커밋 78103bd, 2c97807): 경합 관측(재현 안 됨) + 실패 재시도 구현. 빌드/시험/clippy OK.
...
- T3-T4 완료 (커밋 62a9c37, 0ff96e5): 잠금 소유 이전 + 위키 큐 정정. 리뷰 지적 0건(quality SUGGEST 1).
  - **T3 실측**: 잠금이 실제로 직렬화하면서 `cargo test --lib`가 **2.0초 → 3.9~4.8초**가 됐다(813건). 기준선 15초 안이고 캐시 히트는 잠금을 타지 않는다 — 셸 호출마다 잡는 정상 비용이다. **다음 회차가 옛 값(2.0초)을 기준선으로 인용하지 않도록 위키에 남겼다.**
  - **T3 확인**: 데드락 검사는 `--no-run`으로 빌드를 분리하고 lib 하네스에 60초 상한을 씌워 돌렸다(`moa-*.exe`가 6개라 `--message-format=json`의 `executable`로 특정 — 와일드카드로 고르면 시험 0건짜리 bin 하네스를 집는다).
  - **T4**: `[K-DRIFT]` 2줄(미검증 원인 서술 · 사라진 `shell_test_guard` 심볼명) + `[PROJECT-FACT]` 1줄(재진입 불변식)을 vault `pending.md`에 큐잉했다.

## Next Steps
- 권장 다음 액션: 브랜치 `task/icon-texture-lock`을 master에 병합할지 사용자 승인 (push·병합·태그는 루프 권한 밖)
- Suggested skills: 없음 (이 plan은 완료)
- 위키 갱신: vault `pending.md`에 이 회차 큐 4줄이 대기 중이다(`[K-DRIFT]` 2 · `[PROJECT-FACT]` 2). 위키 세션(`pjc:llm-wiki` 절차 B/M)에서 `moa/conventions.md`에 반영해야 하며, 그때 **미검증 원인 서술 정정**이 핵심이다
