...
- [x] T6. 드롭 대상 패널을 끌고 있는 동안 강조한다
  ...
  - **Files**:
    - 주: `src/ui/app.rs`(판정 + 그리기 호출), `src/ui/splitter.rs`(`draw_drop_highlight` — 굵기·모양 규칙이 그 파일의 private 상수 `PANE_BORDER_WIDTH`라 그리기 자체는 그쪽에 둔다. **`show_layout`의 시그니처는 늘리지 않는다** — Design ③이 막으려는 것은 그 순환이지 파일 자체가 아니다)
...
- T5-T6 완료 (커밋 `73225df`, T6 완료 커밋): 탐색기에서 끌어온 파일을 받는 경로와 드롭 대상 강조를 붙였다. 놓인 자리는 Win32 커서로 재고 물리 픽셀을 논리 pt로 환산한다.
  ...
  - **삽입 위치 부주의가 네 번 반복됐다**(T2·T4·T5·T6). 매번 리뷰가 잡았지만 그 전에 스스로 걸러야 했다 — **새 항목을 넣기 전에 그 자리 바로 위의 주석이 누구를 설명하는지 먼저 읽는다**. `///`(항목 doc)뿐 아니라 함수 본문의 `//`도 같다.
...
> **이 회차는 끝났다** — T1~T8 + Phase F·G 통과. 아래는 **다음 세션이 이어받을 것**이다.

### 이 회차에서 아직 안 한 것 (새 세션이 먼저 확인할 것)

- **수동 검증 4항목이 미완이다** — 드래그의 화면 동작은 이 레포에서 자동 시험으로 재현할 수 없다(대장 2026-08-18 항목, 두 차례 실측). 위 `## Verification Strategy`의 절차 중 특히 ① 전송 후 목록 자동 갱신 ② 셸 복사 대화가 뜬 동안 창이 응답하는지 ③ **화면 배율 125%·150%에서 놓은 패널과 받는 패널이 같은지** ④ **창 밖으로 끌어낼 때 창이 굳지 않는지**(가장 위험한 지점 — 굳으면 T7만 되돌린다).
- **`master` 병합·push·태그·릴리즈를 하지 않았다** — 자율 루프의 권한 밖이라 사용자 승인이 따로 필요하다. 브랜치는 `task/drag-copy-and-transfer-relist`이고 커밋 32개다.
- **위키 반영이 남았다** — `pjc:llm-wiki` 절차 B로 `feat-remote-transfer`·`feat-file-list`에 이번 회차를 반영한다. 구현 세션은 위키를 읽기만 하므로 별도 세션이 필요하다. `[DECISION]`·`[PROJECT-FACT]` 큐에는 이미 적어 두었다(vault `pending.md`).

### 남은 이슈 3건 — **새 세션에서 `pjc:plan-feature`로 계획부터 세운다**

셋 다 이 plan의 범위 밖이라 여기에 task로 넣지 않았다. 상세 근거는 `docs/plans/deferred.md`의 각 항목에 「착수 전 확인된 것」으로 적어 두었으므로 **다시 조사할 필요가 없다**.

| # | 항목 | 성격 | 먼저 필요한 것 |
|---|---|---|---|
| 1 | `ExplorerApp` 채널·상태 필드 캡슐화 | 구조 변경 | 승인. 같은 날 등록된 「충돌 상태 6개 필드」 항목과 **함께** 다뤄야 같은 파일을 두 번 흔들지 않는다 |
| 2 | 드래그 미리보기 그림(`IDragSourceHelper`) | 신규 기능 | 없음. 바인딩·전제(`DRAGDROP_S_USEDEFAULTCURSORS`)는 이미 갖춰져 있다 |
| 3 | 원격 항목을 탐색기로 끌어내기 | 신규 기능(큼) | **PRD 개정이 먼저다** — FR-61이 「로컬 항목만」으로 못박고 있다. 전송 큐와 얽히는 설계 결정이 본체다 |

- 권장 순서: **2 → 1 → 3** (작고 독립적인 것부터. 3은 그 자체로 한 회차다)
- 권장 다음 액션: 새 세션에서 *"남은 이슈 중 <항목>을 계획해 줘"* — `pjc:plan-feature`가 `deferred.md`를 조회해 위 근거를 그대로 쓴다
- Suggested skills: `pjc:plan-feature` → `pjc:implement-task`, 위키 반영은 `pjc:llm-wiki`
