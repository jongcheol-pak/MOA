...
- ~~`src/ui/site_manager.rs`의 모듈 주석이 버튼 수를 옛 값으로 적고 있다~~ → **Phase F에서 고쳤다**: 「사이트 목록 + 버튼 3개 + 그 아래 줄의 `내보내기`·`가져오기` 둘」. 직전 회차가 아랫줄을 더하면서 놓친 것으로, 이번 회차가 그 자식 모듈(`exchange.rs`)을 대대적으로 고쳤기에 부모 서술만 낡은 채 두면 다음 세션을 오도한다.
