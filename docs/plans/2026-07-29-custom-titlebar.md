...
- [ ] T4. 가장자리·모서리 리사이즈 핸들
  ...
  - **Edge Cases**:
    ...
    - 타이틀바 버튼과 핸들이 겹치는 구간(좌측 토글의 좌단 2px, 캡션 버튼의 상단 4px, 우상단 모서리) → **최외곽 4px는 리사이즈가 우선**한다(의도된 동작 — 버튼은 남은 영역으로도 충분히 누를 수 있고, 모서리 리사이즈를 버튼이 먹으면 창 크기 조절이 사실상 불가능해진다)
  - **Halt Forecast**:
    - (i) 내부 위젯과의 입력 충돌 → 핸들 폭 4px·최외곽 한정으로 해소(Risks)
  - **Depends on**: T3

- [ ] T5. 좌측 사이드바 토글 버튼과 우측 설정 팝업(5개 항목, 비활성)
  - **Type**: C
