...
**정적 축 — 코드로 판정되는 19행: 전건 ✅.** 프레임(모서리·배경·폭) · 아이콘 줄(위치·칸 수·아이콘·툴팁·높이·hover·비활성) · 구분선 3자리 · 항목 줄(높이·여백·아이콘 열 20px·라벨·단축키·화살표·hover) · `추가 옵션 표시`(위치·구성·문구)가 모두 구현 파일:라인으로 지목된다(`ui/shell_context_menu.rs`·`ui/widgets.rs`·`i18n/mod.rs`).

**시각 축 — `⏳ 미확인` 3건, F-8로 인계한다.** 데스크톱 창을 띄워야 알 수 있어 이 회차의 자동 검증으로는 닿지 않는다:

| # | 미확인 항목 | F-8에서 확인할 방법 |
|---|---|---|
| V1 | 기준 이미지(`%USERPROFILE%\Desktop\2.png`)와 나란히 놓았을 때 **전체 인상이 같은가** | HUMAN-VERIFY 1 — 실제 우클릭 |
| V2 | 셸이 준 **아이콘이 실제로 그려지는가**(HWND가 있어야 메뉴를 열 수 있어 자동 시험 비대상) | HUMAN-VERIFY 1 |
| V3 | **긴 라벨의 말줄임과 세로 스크롤**이 화면에서 동작하는가 | HUMAN-VERIFY 1 — 확장이 많은 폴더에서 |
...
- [x] T13. 다른 앱이 클립보드를 가져갔을 때도 잘라내기 표시를 푼다 (F-7 M1)
  - **Type**: C
  - **Design**: ① T8 Design ③의 **세 번째 해제 조건**이 T8→T10 인계에서 떨어졌다 — 지금은 붙여넣기 성공과 다른 것 담기 둘만 표시를 푼다 ② `fs::clipboard`에 순수 함수 `cut_marks_for(files: Option<&ClipboardFiles>) -> &[PathBuf]`를 둔다 — **잘라내기로 담긴 것만** 표시 대상이고, 복사로 담겼거나 파일이 아니면 빈 목록이다 ③ `ui::app::clipboard_paste`가 클립보드를 읽은 **직후** 그 값으로 표시를 맞춘다(원격 탭이라 되돌아가기 전에 맞춘다 — 확인 시점은 「붙여넣기를 눌렀을 때」이지 「붙여넣기에 성공했을 때」가 아니다) ④ **비추상화 선언**: 프레임마다 클립보드를 묻지 않는다 — `OleGetClipboard`는 COM 호출이라 매 프레임 도는 자리에 둘 수 없다(plan T8 Design ③의 「붙여넣기·담기 시점에만 확인」 그대로)
  - **Acceptance**: Given 우리가 잘라낸 경로 2개가 표시돼 있고, When 다른 앱이 **복사**로 담은 클립보드를 읽으면, Then 표시 대상이 빈 목록이 된다(순수 함수 시험). Given 다른 앱이 **잘라내기**로 다른 경로를 담았으면, Then 표시 대상이 그 경로로 바뀐다. Given 클립보드에 파일이 없으면(`None`), Then 빈 목록이다
  - **Files**:
    - 주: `src/fs/clipboard.rs`, `src/ui/app.rs`
    - 테스트: `src/fs/clipboard.rs` 내 `mod tests`(세 갈래 — 복사·잘라내기·파일 아님)
  - **Edge Cases**: 클립보드를 다른 앱이 쥐고 있어 읽지 못함 → `take()`가 `None`이라 표시를 푼다(우리 것이 아직 남아 있을 수 있으나, **흐린 채 남는 것보다 푸는 쪽이 안전한 어긋남**이다 — 붙여넣기는 어차피 되지 않는다) / 원격 탭에서 `Ctrl+V` → 표시만 맞추고 붙여넣지 않는다
  - **Depends on**: T8, T10
...
- **Phase G 통과** (2026-08-22, 재루프 0회). F-7이 PRD 전수 대조를 수행했고(Sonnet 대체·체크리스트 대체 아님 — 정상 경로) 그 결과를 재사용한다. **Must 1/1 충족**(FR-8), Should 4/4 충족(FR-12·FR-64·FR-39·FR-25). 미충족 0건이라 갭 처리 없음. 그 밖의 active FR은 `## PRD Coverage`가 「이번 범위 외 (기구현)」으로 명시해 대조 제외.
  - **⏳ HUMAN-VERIFY**: FR-8의 「탐색기와 같은 모양인가」·「설치된 확장이 실제로 뜨는가」와 FR-64의 화면 동작은 창을 띄워야 알 수 있다 — F-8 관문으로 넘긴다.
- **F-7 통과** (2026-08-22, 2라운드). 1라운드 MAJOR 1은 **T13 구현으로** 해소, MINOR 1은 PRD 문면 정정으로 해소, MINOR 1은 `(판정 유보)`라 미등재. 2라운드 판정 BLOCKER 0 · MAJOR 0 · MINOR 1(`(판정 유보)` — 담기 실패 경로의 조건 3, 다음 `Ctrl+V`·`Ctrl+C/X`에서 자가 회복하므로 등재 불요). **재진입 1/3.**
- **F-7 1라운드** (2026-08-22): BLOCKER 0 · MAJOR 1 · MINOR 2. MAJOR(잘라내기 표시의 세 번째 해제 조건 미구현)는 **T13 추가 task로 해소**했고, MINOR 1(PRD FR-12 원격 `F5` 문면)은 문면을 고쳤다. MINOR 2(`move_into`가 사전 승인 목록에 없음)는 `(판정 유보)`라 등재 게이트 ⓐ에 따라 대장에 올리지 않았다 — 사유는 Progress Log의 T10 항목이 이미 든다. **F-7 재진입 1/3**.
...
- T13: 리뷰 지적 수정 사이클 1/5 — 1라운드 spec OK, 품질 MINOR 2(불필요한 복제·불완전한 함수 doc) 반영. **품질 리뷰가 내 판단을 실측으로 뒤집었다** — `.to_vec()`이 빌림 때문에 필요하다고 봤으나 NLL이 알아서 끝내 준다는 것을 재현 코드로 확인해 줬다
- T12: 리뷰 지적 수정 사이클 0/5 — 1라운드 spec OK(MINOR 2는 정보성). 품질 리뷰는 Type A(문서만)라 비대상
...
- **T5-T6 완료** (커밋 `8ef5f2f`·진행 중): Win11 모양 메뉴를 그리고 `ui::app`에 배선했다. 셸 확장 항목·하위 메뉴·`추가 옵션 표시`가 모두 동작하고, 아이콘 줄의 잘라내기·복사·삭제·공유가 실제 기능에 닿는다.
  - **결정**: `이름 바꾸기` 칸만 **비활성**이다(`MenuState::can_rename = false`) — 받을 곳(목록 인라인 편집)이 T8 산출물이다. T5의 「대응 기능이 없는 칸은 비활성」 규칙을 그대로 적용한 것이며, **T8이 그 값을 참으로 바꾼다**.
  - **결정**: `MenuRequest`에 폴더 여부를 **더하지 않았다**(계획과 다름) — 아이콘 줄이 그것으로 가르는 것이 없다. 그래서 계획이 예상한 파일 다섯(`panel`·`splitter`·`shell_host`·`list_details`·`list_grid`)을 손대지 않았고, plan Files를 그 사실로 정정했다.
  - **결정**: 셸 메뉴 배선을 `src/ui/app/shell_menu.rs` 자식 모듈로 뺐다 — `app.rs`가 3000줄을 넘었고 `transfer_conflict.rs`가 같은 패턴의 선례다.
  - **결정**: 창을 숨기는 두 길의 뒷정리를 `hide_window` 한 곳에 모았다 — 각각 적었다가 한쪽을 빠뜨렸고, 리뷰가 2회 연속 그것을 잡았다.
  - **메인이 스스로 찾은 결함 둘**: `menu_size`가 egui 위젯 간격을 빠뜨려 높이를 줄마다 3px 낮게 잰 것, `menu_row` 위임으로 기존 메뉴 셋의 라벨이 20px 밀리던 회귀.
...
- **T8 완료**: 목록에서 바로 이름을 고치고(`Enter` 확정·`Esc` 취소·확장자 앞까지 자동 선택), 잘라낸 항목을 흐리게 보인다. 메뉴 아이콘 줄의 `이름 바꾸기`가 이것을 연다(`can_rename`이 참이 됐다).
  ...
  - **결정**: 편집 취소의 정본을 `PanelState::switch_active_tab` 한 곳에 뒀다 — **탭이 실제로 바뀌는 네 길**(탭 전환·탭 닫기·새 탭·사이트 탭 거두기)만 그리로 온다. 처음에는 `reload_active_tab`에 얹었는데, 그러면 표시 규칙(숨김·시스템) 토글처럼 **탭이 바뀌지 않는 재열거**에서도 입력하던 이름이 사라졌다(품질 리뷰가 잡은 회귀). 감시 갱신(FR-10)은 `start_load`를 곧바로 불러 어느 길도 타지 않으므로 **편집이 유지된다**(plan Edge Case 그대로).
...
- **T9 완료**: 무수식 키(`F2`·`Delete`)를 **마지막으로 누른 영역**이 갖는다 (FR-12). 파일 목록에서 누른 `F2`가 워크스페이스 이름 편집을 열던 문제(`docs/plans/deferred.md` 2026-07-28)가 이것으로 풀린다.
  - **계획 정정 — Design ①**: `KeyOwner` 타입을 `ui::app`이 아니라 **`ui::menu`**에 뒀다. `ui::sidebar`도 이 값을 받아야 하는데 사이드바가 `ui::app`을 참조하면 의존이 거꾸로 선다. **상태(`key_owner` 필드)는 계획대로 `ui::app`이 든다.** 두 리뷰 모두 이 이탈을 타당으로 판정했다.
  - **계획 정정 — Design ③**: 사이드바에 값을 인자로 넘기려 했으나 `show`의 인자가 이미 일곱이라 clippy `too_many_arguments`가 막았다(실측). `set_owns_keys` 세터를 `show` 직전에 부르는 방식으로 했다 — `ui::panel::apply_display_rules`가 같은 이유로 쓰는 선례다.
...
- **T12 완료**: README에 새 컨텍스트 메뉴·인라인 이름 편집·잘라내기 표시·단축키 11종을 적고 소스 트리에 신규 모듈 넷을 넣었다. AGENTS.md에는 셸 메뉴가 UI 스레드에서 도는 이유(STA 제약)를 블로킹 예외에 이름으로 적고 클립보드 시험 스위치(`MOA_TEST_CLIPBOARD`)를 적었다. 대장에서 T1·T7이 해소한 두 항목을 종결로 옮겼다.
  - **함께 정리**: 레포에 남아 있던 비가시 제어 문자 5건(이전 회차 잔존)을 되돌렸다 — 지금 레포 전체 **0건**이다.
  - **함께 정리**: 2026-07-28 「워크스페이스 Delete 키 배정」 항목의 본문이 *"사이드바가 키를 전역으로 보는 현재 구조"*라고 적고 있었는데 T9이 그 구조를 바꿔 거짓이 됐다 — 막던 이유가 사라졌다는 사실만 적고 **되살릴지는 사용자 판단으로 남겼다**(이번 회차 동반 처리에서 사용자가 뺐다).
  - AGENTS.md는 21,247B → 22,182B(+935B). 주입 상한(16KB) 초과는 대장의 2026-08-22 항목으로 남아 있으며 이번에 해소하지 않았다(별건).

- **T13 완료** (F-7 MAJOR M1): 잘라내기 표시의 **세 번째 해제 조건**을 이었다 — 다른 앱이 클립보드를 가져가면 우리 화면의 흐린 줄도 따라 풀린다. T8 Design ③이 규정하고 T10에 넘겼는데 인계에서 떨어져 있었다(탐색기에서 `Ctrl+C`를 누르면 흐린 줄이 영영 남을 자리였다).
  - **결정**: 맞추는 시점은 **붙여넣기를 눌렀을 때**이지 성공했을 때가 아니다 — 원격 탭이라 되돌아가는 갈래보다 앞에 둔다.
  - **결정**: `take()`가 `None`이면(다른 앱이 클립보드를 쥐고 있어 못 읽는 경우 포함) 표시를 **푼다** — 흐린 채 남는 것보다 안전한 어긋남이다(붙여넣기는 어차피 되지 않는다).
  - **함께**: F-7 MINOR m1 — PRD FR-12의 원격 `F5` 문면이 *"원격 메뉴의 같은 항목으로 간다"*고 적고 있었으나 실제 경로는 `refresh_panel`이고 **범위도 다르다**(그 패널 하나 ↔ 그 연결을 쓰는 모든 패널). 코드가 맞으므로 문면을 고쳤다.

- **F-8 종결** (2026-08-22): 시각 축 3건(V1~V3)이 `⏳ 미확인`인 상태로 확인을 요청했으나, **사용자가 화면 확인 대신 `push·master 병합`을 지시**해 그대로 종결한다(F-8은 확인 게이트이지 강제 수정 게이트가 아니다 — `phase-f-detail.md` F-8 4). **셋은 확인된 것이 아니라 확인을 거치지 않은 채 수용된 것이다**: ① 기준 이미지와 나란히 놓았을 때 전체 인상이 같은가 ② 셸이 준 아이콘이 실제로 그려지는가 ③ 긴 라벨의 말줄임과 세로 스크롤이 화면에서 동작하는가. HUMAN-VERIFY 1~7도 미수행이다.
