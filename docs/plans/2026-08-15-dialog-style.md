...
- [SUGGEST] ~~대화 넷의 마무리 블록을 `finish<T>` 헬퍼로 묶기~~ → **Phase F에서 재검토해 그대로 두기로 했다.** 실측 반복은 `match shell.clicked` 6곳(`remote_menu` 4 · `app` 1 · `remote_states` 1)이지만 **결과 타입과 마무리가 제각각**이다 — `DialogOutcome<T>` 반환(넷) · `Option<bool>`(워크스페이스 삭제) · `Option<HostKeyDecision>`이며 `should_close`를 아예 쓰지 않는 것(호스트 키). 헬퍼가 감출 수 있는 것은 `if closed { return Cancelled }` 세 줄뿐이고, 그것을 함수 뒤로 숨기면 "이 대화는 Esc를 어떻게 다루는가"가 호출부에서 보이지 않게 된다 — AGENTS.md의 "약간 장황해도 동작이 한눈에 보이는 코드" 원칙에 어긋난다 (T2 quality S1 · Phase F m7)
- [SUGGEST] 규약 시험 `대화는_모두_이_모듈을_거친다`가 `src/ui` **바로 아래 파일만** 훑는다(`read_dir` 비재귀) — `src/ui/panel/`에 팝업이 생기면 규약이 조용히 샌다. 같은 파일의 아이콘 시험도 비재귀라 둘을 함께 고치는 편이 낫다 (Phase F m1)
...
- [x] T4. 앱 설정 대화 푸터 교체 (`ui/settings_dialog.rs`)
  - **Type**: C
  - **Acceptance**:
...
- **수동 검증 (⏳ 미확인 — 빌드·시험으로 판정 불가, 사용자 화면 확인 대기)**:
  화면을 띄워야 판정되는 항목이라 task 체크박스가 `[x]`여도 **아래는 아직 확인되지 않았다** —
  모서리가 둥근가 · 버튼이 좌우 끝까지 닿고 균등한가 · 구분선이 보이는가 · 주 버튼이 굵어 보이는가 ·
  hover 채움이 둥근 모서리 밖으로 새지 않는가 · 사이트 관리자 오류 줄과 폼 · 영어 라벨의 칸 넘침.
  ...
  4. 사이트 관리자에서 이름을 비운 채 `확인`을 눌러 오류 문구가 버튼 줄 위에 뜨는지 본다
  4-0. 오류가 **없을 때** 그 자리가 빈 줄로 남는 것이 어색하지 않은지, **일반·고급 탭 모두** 폼 마지막 행이 574px 안에서 잘리지 않는지 본다 (여유 계산은 일반 탭 6행 기준이었다)
  4-1. 앱 설정의 `닫기`가 굵게 나오는데, 대비할 취소 버튼이 없어 굵기가 어색하지 않은지 본다 (T4 quality 확인 요청 — 어색하면 `ButtonSpec::plain`으로 바꾼다)
...
- T3-T4 완료 (커밋 c7812bd, 54356a7): 워크스페이스 삭제·호스트 키 → 앱 설정.
  - 결정: 닫기 판정이 대화마다 갈린다 — 워크스페이스 삭제는 셸의 `should_close`(Esc + 배경 클릭)에 맡기고, 호스트 키는 종전처럼 버튼을 눌러야만 결정이 나간다(배경을 잘못 눌러 연결이 거절되면 사유를 알기 어렵다). 후자는 사유를 코드 주석에 남겼다.
  - 결정: 앱 설정의 `닫기`를 굵게(주 동작) 표시했다 — 대비할 취소가 없어 굵기가 어색할 수 있어 화면 확인 항목(수동 검증 4-1)으로 등록했다.
- T5-T6 완료 (커밋 1f90bb2, 6cff7d4): 사이트 관리자 + 규약 시험 활성화 → 문서 갱신.
  - 결정: 사이트 관리자 바닥을 두 줄로 나눴다(오류 22 + 버튼 44 = 66). 본문이 582 → 574px로 8px 줄지만 일반 탭 폼이 약 280px라 여유가 있다.
  - 결정: `widgets::primary_button`과 고아가 된 색 셋을 지웠다 — bin+lib라 죽은 `pub` 항목을 린트가 잡지 못해 계획이 직접 처리했다. `PRIMARY_FILL`은 전송 큐가 계속 쓴다.
  - 결정: 규약을 문서가 아니라 **활성 시험**으로 강제한다(`대화는_모두_이_모듈을_거친다`) — AGENTS.md에 그 시험 이름까지 적었다.
