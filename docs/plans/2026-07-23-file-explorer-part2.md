...
### D18. 세션 복원 시 삭제된 탭 경로 처리 (T4 구현 중 결정 — 리뷰 B1)
- **Options**: A) 시작 시 각 탭 경로 is_dir() 검사 후 홈 대체 (원안 Edge) / B) 경로를 그대로 신뢰하고 사라진 경로는 기존 열거 실패 경로(fail_pending — 오류 문구·탭 유지)에 위임
- **Chosen**: B
- **Rationale**: A는 UI 스레드 동기 파일시스템 호출(AGENTS DO NOT 위반) — 무응답 네트워크 경로면 시작이 탭 수만큼 누적 블로킹된다(quality 리뷰 BLOCKER B1). B는 기존 실패 모델과 일관되고 블로킹 0. 탭 수 유지라는 Edge의 본래 목적은 동일하게 충족.
- **Source**: AGENTS.md UI 스레드 원칙 + T4 quality 리뷰 B1
...
- [x] T4. 세션 저장/복원 (FR-11)
  ...
  - **Edge Cases**:
    ...
    - 저장된 탭 경로가 삭제됨 → 탭 수 유지, 해당 탭은 표시/전환 시 열거 실패 문구(기존 D6 경로) — 원안 "홈 폴더 대체"는 시작 시 UI 스레드 동기 존재 검사(AGENTS 원칙 위반, 리뷰 B1)라 기각 (D18)
