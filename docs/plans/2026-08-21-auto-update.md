...
- **`AGENTS.md`가 SessionStart 주입 상한(16KB)을 넘는다** — 이번 회차에 릴리즈 절차·노트 규약을 더해 18,420B → 20,601B가 됐다. **이번 변경이 만든 문제는 아니다**(작업 시작 시점에 이미 18.4KB로 넘어 있었고 세션 컨텍스트가 그 경고를 냈다). `pjc:record-project-fact`의 「주입 상한 점검·이관」으로 큰 절을 별도 문서로 옮기고 포인터만 남기는 정리가 필요하다.
...
**Source**: 개발 실행은 `target/debug\`에서 돌아 거기에 `update\` 폴더가 생기면 지저분하고, 설치는 어차피 다른 자리(`%LOCALAPPDATA%\Programs\MOA`)에 되어 지금 도는 exe와 갈린다. 설치본이 아니면 **확인 자체를 하지 않고** 설정 메뉴의 `업데이트` 항목을 비활성으로 둔다. **`릴리즈 노트`는 그때도 활성이다** — 브라우저를 여는 것뿐이라 설치본과 무관하고 FR-63에 그 조건이 없다(조건이 붙는 것은 FR-62뿐이다).
...
- [x] T9. 문서 — 릴리즈 발행 규약(AGENTS)·배포 줄 정정·README
  - **Type**: A
...
- **첫 릴리즈 발행 (2026-08-22, 사용자 승인)** — `task/auto-update` push · 태그 `v0.1.1` · GitHub 릴리즈 `v0.1.1`에 `MOA-Setup-0.1.1.exe`(3,002,257B) 첨부, 본문은 D15 형식(사용자용 요약 + 접힌 SHA256).
  - **앱이 쓰는 경로를 실제 응답으로 검증했다** — `releases/latest`가 `tag_name: v0.1.1`, 자산 `MOA-Setup-0.1.1.exe`, 접기 안의 체크섬 추출(`c35ac539…fff37`, 산출값과 일치), 다운로드 URL을 전부 정상으로 준다. 버전 비교·자산 선택·체크섬 추출이 가짜 표본이 아니라 **진짜 릴리즈에서** 성립함을 확인.
  - **발행 중에 릴리즈 절차의 결함을 하나 찾아 고쳤다** — 버전을 올리면 `Cargo.lock` 지문이 바뀌어 `gen_licenses` 재생성이 필요한데 T9에 적은 순서에 그 단계가 없었다. 실제로 `cargo test`가 깨져서 알았고 AGENTS 절차에 2번 단계로 넣었다.
- **HUMAN-VERIFY 미완** — A(1~6)는 지금 확인 가능. **B(7~14)는 이제 가능하다** — `target/installer/MOA-Setup-0.1.0.exe`(자동 업데이트 코드가 든 0.1.0)를 설치하고 실행하면 v0.1.1을 새 판으로 본다.
