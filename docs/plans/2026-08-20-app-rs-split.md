...
- **Phase F 통과 (HEAD `b890384` + 후속 정정)** — F-1 `cargo build` OK · F-2 **866/866**(BASE와 시험 개수 동일 — 소실·추가 0) · F-3 clippy·fmt 경고 0. F-7: BLOCKER 0 / MAJOR 1 / MINOR 3, 전건 처리했다.
  - **M1(수용)** — T4 acceptance가 명시한 신규 Deferred 3건 중 **`app/tests.rs` 분리 1건이 대장에 빠졌다**. 그 자리에서 대기에 추가했다(64 → 66 → **67건**).
  - **m3(수용)** — Progress Log의 「49개가 전부 `pub(super)`」가 사실과 달랐다(47개가 `pub(super)`, `connect_site`는 `pub`, `apply_drop`은 private). 정정했다.
  - **m1(조치 불요)** — 옮긴 시험에 주석 3줄이 늘었다. T2 acceptance m4가 지시한 경로 참조를 설명하는 주석이라 리뷰어도 내용상 정당하다고 판정했다.
  - **m2(다음 회차 권고)** — 필드 가시성 검사 `grep -cE "^    pub"`가 4칸 들여쓴 `pub fn`도 세어 전후 비교가 3 → 2로 달라졌다(`connect_site`가 파일을 떠난 결과). **실제 `ExplorerApp` 필드는 전후 모두 private 0건**임을 F-7이 확인했다. 다음에 같은 acceptance를 쓸 때는 구조체 정의 범위로 좁힌다.
  - **F-7이 독립 검증한 것**: 전체 소스를 정규화해 다중집합 대조한 결과 **로직 라인 차이 0**(차이는 `use` 재구성·rustfmt 재개행·주석뿐) · 49개 심볼 정의 잔존 0 · `pub(crate)` 이상 0건 · T3의 재귀가 `src/ui` 하위 두 폴더를 모두 훑음.
  - **Phase G 미적용** — `**PRD**:` 줄이 없다. F-7이 `docs/prd.md`에 `app.rs`·모듈 구조 언급이 **0건**임을 확인했다.
  - **F-8 미적용** — `## 시각 요소 분해` 섹션이 없다(파일 배치만 바뀐 회차).
  - **HUMAN-VERIFY 없음** — 화면·동작이 바뀌지 않아 사람이 확인할 축이 발생하지 않았다.

## Retry Ledger

## Next Steps

- 권장 다음 액션: `task/app-rs-split` 브랜치를 master로 병합할지 결정 (push·병합은 별도 승인 대상). **직전 회차(`task/small-refactors`)도 master 미병합이며 이 브랜치가 그 위에 쌓여 있다.**
- Suggested skills: 공식 `/code-review` (병합 전 최종 점검을 원할 때)
- 위키 갱신: `[PROJECT-FACT]` 2건·`[DECISION]` 1건을 vault `pending.md`에 큐로 남겼다. 본문 반영은 별도 세션(`pjc:llm-wiki` 절차 B)

## Progress Log

- **T3-T4 완료**: 비재귀 규약 시험 둘을 재귀로 + 대장 정합.
  - **T3이 만든 것은 검증의 회복이다** — T1·T2가 새 폴더를 만들면서 두 시험의 사각지대가 생겼고, 그 안에 대화를 그리는 코드가 들어갔다. 재귀화 후 `src/ui/app/`에 금지 아이콘을 담은 임시 파일을 넣어 **실제로 잡히는 것을 확인**했다(비재귀였다면 놓쳤다).
  - **quality 리뷰가 stale 서술 둘을 짚었다** — `theme.rs`와 `AGENTS.md`가 「모달 규약 테스트는 비재귀」라고 적고 있었는데 이번 변경으로 틀린 말이 됐다. 자기 유발이라 그 자리에서 고쳤다.
  - 대장: 대기 **64 → 66건**(종결 2 · 신규 4). `app.rs` 항목은 종결하지 않고 **남은 책임 목록으로 갱신**했다 — 두 책임이 빠졌을 뿐 ①은 여전히 걸린다.

- **T1-T2 완료** (커밋 `421e450`, T2는 아래 완료 커밋): `ui/app.rs`가 **3802 → 2356줄**이 됐다. 충돌 확인 487줄·원격 배선 1038줄이 자식 모듈로 나갔다.
  - **가시성을 한 곳도 넓히지 않았다** — 옮긴 49개 중 **47개가 `pub(super)`**, `connect_site`는 원래대로 `pub`, `apply_drop`은 자식 내부 전용이라 private이다. `ExplorerApp` 필드는 그대로 private이고 `pub(crate)` 이상으로 열린 것이 0건이다(F-7이 m3으로 이 문장의 「49개 전부」를 정정했다). 자식 모듈 구조가 그것을 가능하게 했다(계획 단계에서 세 방향을 별도 crate로 컴파일해 검증한 것이 그대로 맞았다).
