...
- F-8 대기 — 화면 확인 13항목이 사용자 확인을 기다린다 (완료 선언 보류)
...
| NFR-1 콜드 스타트 | — | ⏳ HUMAN-VERIFY | 릴리즈 빌드로 실측 필요 |
| NFR-2 유휴 메모리 | — | ⏳ HUMAN-VERIFY | 자세히 보기 기준, 화면 확인 항목 |
| NFR-3 10만 파일 무정지 | — | ⏳ HUMAN-VERIFY (13항목 #12) | 목록 보기 가상 스크롤은 F-7 M1에서 수정 |
| **NFR-9 썸네일 캐시 상한** | — | ✅ 자동 + ⏳ 일부 | 픽셀 상한은 단위 테스트로 강제(장수 AND 바이트), GPU 텍스처·총 Working Set은 #13 |
