...
| NFR-1 콜드 스타트 | — | ✅ **충족 (0.4~0.6초, 기준 1초)** | 조사 도중 한때 1.2~10.3초로 측정돼 위반으로 보고했으나, `log` 로거를 붙인 뒤 같은 조건이 0.3~0.7초로 재측정됐고 이후 어떤 방법으로도 느린 값이 재현되지 않았다(순정 릴리즈·빌드 직후·새 exe 복사본·간격 3~60초, 24회 이상). 구간 내역: glutin Display 생성 260~420ms가 대부분, 우리 코드는 합쳐 약 50ms. 느린 수치의 정체는 미특정 — Deferred에 기준선과 함께 남겼다 |
...
**NFR-1도 최종적으로 충족이다.** 조사 도중 위반으로 보고했다가 재측정에서 뒤집혔다 — 자세한 경위와 초기화 구간 기준선은 Deferred 대장에 남겼다.
