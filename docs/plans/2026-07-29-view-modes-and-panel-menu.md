...
| NFR-1 콜드 스타트 | — | ❌ **위반 (1.2~10.3초, 기준 1초)** | 임시 계측으로 구간 특정: `main`~`run_native` 직전 **14ms**, 나머지 전부가 `run_native`~`ExplorerApp::new` 진입 구간(**eframe 창 생성 + GL 컨텍스트 초기화**)이다. 우리 코드는 합쳐 50ms(폰트 20·아이콘 16·COM 12). 창 옵션 4조합·실행 간격 20초·네트워크 드라이브 가설 모두 측정으로 배제했고, 값이 8배 흔들려 시스템·드라이버 요인으로 보인다. **이번 작업이 만든 문제가 아니고 시작 경로를 건드리지도 않았다** — 배제된 가설·남은 후보와 함께 Deferred 등재 |
...
**Must 충족률 100%** (FR-1~8·15~17 전부 ✅). 기능 갭 없음 — 재루프 불필요.

**NFR-1은 위반 상태로 남는다.** 이번 작업의 산물이 아니고(시작 경로 무변경) 원인이 우리 코드 밖(eframe 창·GL 초기화)이라 이 plan에서 고칠 수 없다 — Deferred 대장에 조사 항목으로 등재했다. Phase G의 판정 대상은 이번 plan이 커버로 선언한 FR/NFR이며, NFR-1은 그 대상이 아니다.
