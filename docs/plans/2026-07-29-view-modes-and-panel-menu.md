...
| NFR-1 콜드 스타트 | — | ❌ **위반 (7.7~9.6초, 기준 1초)** | 임시 계측으로 구간 특정: `main`~`run_native` 직전 **14ms**, `run_native`~`ExplorerApp::new` 진입 **7.7~9.6초**. 우리 코드가 아니라 **eframe 창 생성 + GL 컨텍스트 초기화** 구간이다(폰트 20ms·아이콘 16ms·COM 12ms). 연속 3회 개선 없음 → 캐시 성질 아님. **이번 작업이 만든 문제가 아니고 시작 경로를 건드리지도 않았다** — 별도 조사로 Deferred 등재 |
