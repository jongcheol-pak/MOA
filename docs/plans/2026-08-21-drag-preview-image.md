...
| 8 | **셸이 만들어 준 `IDataObject`가 임의 사설 형식의 `SetData`를 받는다** | 헬퍼가 그것을 **요구한다**는 것은 MS 문서로 확인했으나, `SHCreateShellItemArrayFromIDLists` → `BindToHandler(BHID_DataObject)`가 준 객체가 실제로 받아 주는지는 **실행해 봐야 안다** | ✅ **2026-08-21 확인** — 수동 검증 1에서 그림이 실제로 떴다(설명 문구 포함). 셸 데이터 객체가 헬퍼의 사설 형식을 받는다 |
| 9 | `hbmpDragImage`를 누가 지우는가 | MS 문서에 서술이 없다(확인 ③) | ✅ **2026-08-21 확인** — 20회 끌었다 놓기에서 GDI 개체 수가 계단식으로 늘지 않았다. D6의 「성공하면 지우지 않는다」가 옳고 누수도 없다(헬퍼가 얹는 시점에 비트맵을 읽어 가는 것으로 보인다) |
| 10 | 드래그의 화면 동작은 자동 시험으로 고정할 수 없다 | Deferred 대장 2026-08-18 항목(두 차례 실측 실패) | ✅ |

## Risks & Unknowns

- ~~**R1. 그림이 아예 안 붙을 수 있다**~~ → **2026-08-21 해소**(수동 검증 1 통과). 종전 서술: (전제 8). 셸 데이터 객체가 사설 형식을 거부하면 그렇다. 그때도 드래그·복사는 종전대로 되며 그림만 없다. 수동 검증 1에서 바로 드러난다.
- ~~**R2. GDI 핸들 누수**~~ → **2026-08-21 해소**(수동 검증 5에서 증가 없음). 종전 서술: (전제 9). 소유권이 문서에 없어, 지우지 않는 쪽을 택하면 헬퍼도 안 지울 때 끌 때마다 비트맵 한 장이 남는다. 수동 검증 5로 20회 실측해 계단식 증가가 있으면 D6을 뒤집는다.
...
- **Phase G 통과 (Must 100%)** — 이번 회차가 커버 대상으로 선언한 것은 FR-61 ⓑ(Should) 하나이며 코드 경로로 충족된다. **active Must FR 미충족 0건**이라 재루프 사유가 없다. 다만 FR-61의 검증 방법이 정한 대로 화면 확인은 HUMAN-VERIFY였고, **2026-08-21에 사용자가 수동 검증 6항목을 모두 통과로 확인**했다 — 그림·설명 문구·다중 선택·`Esc` 취소·GDI 개체 수·원격 항목 비대상. 이로써 계획의 미확인 전제 둘(8·9)이 함께 닫혔다.
