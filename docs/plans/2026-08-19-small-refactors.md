...
- **T5-T7 완료**: `show_remote_node` 인자 축소 + `ConflictDecision` 3상태화 + 대장 정리.
  - **대기 항목 수 실측: 78 → 65건**(제거 15줄 − 신규 이관 2건 = 13 감소). **처리한 주제는 14개인데 제거된 줄은 15개**다 — `gen_licenses` 중복 제거 항목이 2026-08-18·2026-08-19 두 날짜로 이중 등재돼 있었기 때문이다(둘 다 종결로 옮겼다).
  - **T5는 대장 수치가 틀렸다** — 대장이 「9개 → 6개」라 적었으나 묶는 대상이 셋이므로 9−3+1=**7개**다. plan 단계의 리뷰가 이 산술 모순을 BLOCKER로 잡아 acceptance를 고친 뒤 구현했다.
  - **T7의 종결 사유를 두 갈래로 갈랐다** — 「사실 오류로 무효」 1건(트리 메뉴 보정: 본받으라던 원격 쪽도 어림값)과 「판단상 기각」 7건(서술은 참이지만 고치면 손해). 처음에는 5건을 「사실과 어긋남」으로 묶었는데 plan 리뷰가 그중 둘(`nav_button`·빈 영역 클릭)은 대장 서술이 지금도 정확함을 실증해, 그대로 갔으면 **영구 기록에 거짓 사유가 남을 뻔했다**.
