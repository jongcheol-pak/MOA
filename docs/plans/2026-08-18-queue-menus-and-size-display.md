...
| 라벨 배치로 바꾸면서 로그 줄 높이(17px)·열 폭(62/44px)이 미세하게 달라질 수 있다 | 「로그 치수는 원본과 같다」 시험과 화면이 어긋난다 | **실제로 일어났다** — 커서 배치가 열 x를 밀었다(T6 Design ① 참조). 자리를 절대 좌표로 잡고, 그려진 x를 재는 시험을 새로 두어 `10/82/136`을 못 박았다 |
...
- [x] T1. 크기 표기를 한 규칙으로 통일한다 (소수 둘째자리 · KB/MB/GB 자동)
  ...
  - **Edge Cases**:
    ...
    - **단위 승격 경계의 반올림** — 1MB 미만이라 KB 갈래로 가는데 반올림하면 `1024.00 KB`가 되는 구간이 있다. 판정을 **반올림한 값 기준**으로 한다 — 그 단위의 값이 반올림 후 `1024.00` 이상이면 한 단계 올린다. 실제 경계는 **1,048,571바이트**다(1,048,570은 1023.9941 → `1023.99 KB`로 남는다 — 계획 당시 예시로 적은 값이 한 칸 어긋나 있었다). GB 위로는 올릴 단위가 없어 그대로 둔다
...
- [x] T6. 서버 로그를 끌어 선택·복사할 수 있게 한다
  ...
  - **Halt Forecast**:
    - (i) 라벨 배치로 치수가 어긋남 → **실현됐고 해소했다**: 커서 배치(`add_sized`·`allocate_ui_with_layout`)가 x를 밀어 절대 좌표로 바꿨고, 그려진 x를 재는 시험이 지킨다(Design ①)
  - **Depends on**: -
...
- Phase F 통과 (HEAD `b5033ad` 기준 — F-7 `plan-completion-reviewer`: BLOCKER 0 · MAJOR 0 · MINOR 5. 그중 넷(plan 예시 숫자·Risks/Halt 문면 2·시험 주석)은 자기 유발이라 이연 없이 정정했고, 큐 표 상시 구분선 시험은 등재 게이트 ⓑ(범위 안)에 따라 그 자리에서 추가했다. 남은 하나(상단 도크 탭의 site 축)는 사용자 결정으로 범위 밖이라 대장에 등재)
- Phase G 통과 (Must 100% — FR-4·FR-36·FR-40이 이번 커버 대상이고 개정 문면과 구현이 일치, FR-11 세션 복원 무회귀 확인. 갭 0이라 재루프 없음)
...
- T5-T6 완료 (커밋 7d827b9, 다음): 파일 목록 머리글에 구분선·가이드를 넣고, 서버 로그 세 열을 고를 수 있는 라벨로 바꿨다. 833건 통과.
  - 함정(둘 다 실측으로 잡았다): ① egui의 **커서 배치는 열 x를 민다** — `add_sized`는 위젯을 셀 가운데에 놓고(`10/82/136 → 41/104/363`), `allocate_ui_with_layout`은 요청 폭이 아니라 내용 크기만 차지한다(`10/20/30`). 고정 열은 **절대 좌표 + `new_child`**로 잡아야 한다. ② 리팩터링 뒤 **시험이 죽은 경로를 검증**하고 있었다(`elided_galley_colored`를 직접 부르는 색 시험) — 실제 렌더 셰이프에서 색을 재도록 다시 썼다(두 리뷰어가 함께 지적).
  - 계획 정정: T5의 드래그 가이드 자동 시험은 egui 끌기 판정을 `RawInput`으로 재현하지 못해 뺐다(두 차례 시도). T6 Design ①·②는 위 ① 때문에 수단이 바뀌어 plan 문면을 실제 구현에 맞췄다.
...
- 권장 다음 액션: **사용자 화면 확인**(`## Verification Strategy` 수동 검증 1~5) → 이상 없으면 `master` 병합·push 승인
- 자동 검증이 닿지 못한 축: 드래그 가이드가 행 바닥까지 끊김 없이 이어지는가 · 로그 여러 줄 드래그 복사문 · 재시작 후 큐 열 폭 유지 · 새 크기 표기의 실제 가독성
- Suggested skills: 공식 `/code-review` (병합 전), `pjc:llm-wiki`(위키 반영 — 사용자 동의 시 별도 세션)
