...
- **하한**: 핸들이 있는 여섯 열이 40px — `list_details::MIN_COL_WIDTH`(`list_details.rs:27`, 이미 `pub`이고 같은 `ui` 계층)를 그대로 쓴다(신설하지 않는다). 흡수는 "합 < 표 폭"일 때만 일어나고, 합이 표 폭을 넘으면 **오른쪽이 잘린다**(가로 스크롤은 Out of Scope) — 사용자가 폭을 줄이면 돌아온다.
...
- [ ] T5. 파일 목록 머리글에 열 구분선과 드래그 가이드를 둔다
  ...
  - **Files**:
    ...
    - 테스트: `src/ui/list_details.rs` `mod tests`(드래그 중 가이드 x가 산출되는 것을 세는 시험 1건 — 셰이프 수집 방식은 같은 파일의 기존 시험을 따른다)
  - **Edge Cases**:
...
- [ ] T6. 서버 로그를 끌어 선택·복사할 수 있게 한다
  - **Type**: C
  - **Design**: ① `ui::log_panel::show_line`이 painter 직접 그리기 대신 라벨 셋을 놓는다 — 행 rect를 `allocate_exact_size`로 잡아 오류 배경을 먼저 칠하고, 그 안에 child `Ui`를 만들어 `add_sized([TIME_WIDTH, LINE_HEIGHT], Label::new(RichText::new(시각).monospace().size(FONT_PX).color(…)).selectable(true))`를 시각·종류·본문 순으로 놓는다(본문은 남는 폭 전부 + `.truncate()`). **간격을 손으로 맞춘다** — child `Ui`의 `spacing_mut().item_spacing.x = COLUMN_GAP`(10px)로 두고 왼쪽 시작을 `PAD_X`(10px) 들여 현행 x 좌표(`시각 10 · 종류 82 · 본문 136`)를 그대로 재현한다. egui 기본 간격(4px)을 그냥 두면 열이 밀려 치수 시험이 깨진다. ② 신규 심볼 없음 — 기존 함수의 본문만 바뀐다. ③ `list_common::elided_galley_colored`는 이 함수에서 더 쓰지 않는다(다른 호출부는 그대로). ④ 비추상화 선언 — "선택 가능한 표 줄" 부품을 만들지 않는다(로그 한 곳뿐이다).
