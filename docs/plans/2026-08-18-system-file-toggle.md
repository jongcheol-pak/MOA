...
- [ ] **T3. 설정 화면에 토글을 세우고 문구를 바꾼다** — Type C
...
- [ ] **T4. 문서를 실제와 맞춘다** — Type A
  - **Files**: `README.md`(설정 화면 `파일 보기` 서술), `docs/prd.md`(FR-13 + 변경 이력)
  - **Acceptance**: ① README의 `파일 보기` 줄이 토글 셋을 적고, 시스템 토글의 기본값이 꺼짐이라는 것과 두 속성이 함께 붙은 항목 규칙이 드러난다 ② PRD FR-13이 두 토글로 서술되고 `## 변경 이력`에 2026-08-18 줄이 선다 ③ 문서에 없는 기능을 적지 않는다(역대조)
