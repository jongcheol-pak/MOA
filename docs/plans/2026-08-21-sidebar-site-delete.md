...
- Phase F 통과 (HEAD 0577aa5) — 빌드 OK · 시험 962/962 · clippy 경고 0 · fmt OK. F-7(plan-completion-reviewer)이 MAJOR 2·MINOR 1을 냈고 셋 다 자기 유발이라 그 자리에서 해소했다(열지 않은 워크스페이스의 잔존 탭 · stale 주석 2자리).
- Phase G 통과 (Must 100%) — 이 plan이 커버 대상으로 선언한 FR은 FR-29(Must) 하나이며, 개정 문면의 다섯 요소(연결 해제 · 원격 탭 닫기와 마지막 패널 로컬 폴백 · 큐 항목 제거 · 조건부 확인 · 사이트 기록 보존)가 `detach_site` 1~5단계와 1:1로 대응함을 대조했다. **active Must FR은 이번 범위 밖(기구현)이고 회귀도 없다** — 탭이 서는 판정 규칙(FR-36)·마지막 패널 유지(FR-2)·탭↔연결 계약(FR-32)을 건드리지 않고 그 입력만 없앴으며 시험 962건이 전부 통과했다.
...
- 권장 다음 액션: 아래 HUMAN-VERIFY 5건을 실제 앱에서 확인 → 이상 없으면 push·릴리즈 여부 결정(별도 승인 필요)
- Suggested skills: 공식 `/code-review`, `pjc:llm-wiki`(feat 페이지 ingest — 큐에 `[K-DRIFT]`로 대기 중)

- 권장 다음 액션: 승인 후 `pjc:implement-task`로 T1부터 실행

## HUMAN-VERIFY (기계로 확인할 수 없는 것)

- **전송 중 삭제** — 사이트에 연결해 원격 탭을 열고 파일 전송을 건 뒤 사이드바에서 그 사이트를 지운다 → 확인 대화가 뜨고 `삭제`를 누르면 도크의 연결별 탭에서 그 사이트가 **즉시** 사라지는가.
- **전송 없는 삭제** — 전송이 하나도 없는 사이트를 지운다 → 확인 없이 곧바로 사라지는가.
- **마지막 패널** — 원격 탭이 창의 유일한 패널의 유일한 탭일 때 지운다 → 로컬 시작 폴더 탭이 그 자리에 서는가.
- **파괴색** — 우클릭 메뉴의 `삭제` 줄에 마우스를 올리면 빨간 배경(#C42B1C)이 칠해지는가.
- **`.part` 정리** — 받는 중이던 파일의 `.part` 조각이 남지 않는가(자동 시험이 없는 유일한 항목).
