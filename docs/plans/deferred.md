...
- [2026-08-19] 앱 이름 `"MOA"` 상수 통합 — `autostart`(레지스트리 값 이름)·`settings`(폴더 이름)·`tray`(툴팁)·`main`(창 제목)이 각자 든다. 넷은 화면 문구가 아니라 언어를 따라가면 안 되는 값이라 i18n 카탈로그와는 별개다 (출처: 2026-08-19-about-dialog D8)
