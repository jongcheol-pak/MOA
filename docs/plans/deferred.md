...
- [2026-08-17] **`IconTextures`의 아이콘 변환이 셸 잠금 밖이다** — `icon_tex::icon_to_image`의 `ImageList_GetIcon`은 `fs::icons::shell_test_guard`가 감싸지 않아 **병렬 시험에서 경합**하고, 실패한 인덱스를 `None`으로 **영구 기억**한다(재시도 없음). 실측: T5의 배지 시험이 병렬 판에서만 실패하고 `--test-threads=1`에서는 통과했다. 그 회차는 배지를 텍스처와 분리해 우회했으나 **아이콘 자체를 보는 기존 시험들은 여전히 노출**돼 있다. 근본 해결은 그 변환도 같은 잠금 안에서 돌게 하는 것 (출처: 2026-08-17-network-drive-status T5)
