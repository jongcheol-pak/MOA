...
- [2026-08-18] 라이선스 전문 라벨이 **파일 이름에 기대는 구간이 남아 있다** — `GPLv2`·`LIBM-MIT`·`UNICODE`·`BSD`는 `normalize_label`의 매핑에 없어 그대로 화면에 뜬다. 표기가 갈리는 것(`APACHE`·`mit` 등)만 맞췄고 나머지는 파일 이름이 더 정확하다고 보았다. 라이선스 텍스트를 실제로 판별하는 길이 생기면 다시 볼 자리 (출처: 같은 plan T6 · F-7 m1)
...
## 종결
- [2026-07-29 → 2026-08-18] 설정 팝업 네 항목 중 **`오픈소스 라이선스`** → **반영**(2026-08-18, plan `2026-08-18-open-source-licenses.md`). 타이틀바 설정 메뉴의 그 항목이 좌우 2단 고지 화면을 연다 — 구성 요소 158개(대상 155 + 정적 링크·동봉 3)의 이름·버전·SPDX·저작권과 라이선스 전문 95벌. 고지 자료는 `examples/gen_licenses.rs`가 만들어 `assets/licenses.json`으로 커밋되고, 낡으면 `Cargo.lock` 지문 대조 시험이 잡는다. **대기 항목은 남은 셋(업데이트·릴리즈 노트·정보)으로 좁혔다**
