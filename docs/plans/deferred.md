...
- [2026-08-05] **원격 목록에 문자셋 적용 (FR-46의 남은 절반)** — `remote::charset`과 `문자셋` 탭·저장은 됐으나 목록 경로에 잇지 못했다. `suppaftp`가 LIST 줄을 `from_utf8_lossy`로 이미 디코딩하고 `ssh2`는 `from_utf8().unwrap()`으로 옮겨 **원본 바이트가 오지 않는다** — 저수준 스트림 직접 파싱 또는 SFTP 크레이트 교체가 필요 (출처: 2026-08-04-ftp-integration T16)
...
- [2026-08-05] `ui/app.rs`(2400줄)·`ui/panel.rs`(2100줄)·`ui/site_manager.rs`(1760줄)가 1500줄 분리 검토선 초과 — 각각 단일 책임이라 이번엔 쪼개지 않았다. 원격 배선을 떼는 쪽으로 재검토 (출처: 2026-08-04-ftp-integration T10·T16·T23 quality 리뷰)
