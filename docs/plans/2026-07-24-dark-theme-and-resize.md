...
- 후속 수정 (헤더·탭 다크 재작업, 사용자 재보고): T3 헤더·T6 탭이 실기기에서 밝게 잔존. 근본 원인 정정 — ① 헤더 NM_CUSTOMDRAW는 ListView로만 가고 패널로 forward 안 됨(Phase 4의 "패널 WM_NOTIFY 처리"가 오판) ② 탭 오너드로우는 항목만 그려 스트립 여백이 밝게 남음. 검증 레퍼런스 ysc3839/win32-darkmode에 따라 재구현: 헤더=`SetWindowTheme("ItemsView")`+ListView 서브클래스(list_dark_proc)로 헤더 통지 가로채 글자색 지정, 탭=서브클래스(tab_dark_proc) WM_ERASEBKGND로 스트립 배경 WINDOW_BG 도색. `draw_header_dark`·패널 헤더 분기 제거. build/clippy/fmt/test(75+2) OK. 실렌더링 HUMAN-VERIFY. 상세: debug-2026-07-24-dark-ownerdraw.md Phase 5.
...
- **범위 확장(구현 중 추가)**: 주소창이 얹히는 **패널 컨테이너 배경**(자식 컨트롤 사이 여백)과 **상태 라벨**도 함께 다크화한다 — 패널 배경이 밝으면(`COLOR_BTNFACE`) 목록·트리·주소창 사이 여백이 밝게 남아 "탐색기 다크"가 미완성이 되기 때문. 패널 클래스 배경 브러시(WINDOW_BG) + `WM_CTLCOLORSTATIC`(상태 라벨) 처리. Files에 이미 panel.rs 포함, "탐색기 전부 다크" 취지 내 확장.
