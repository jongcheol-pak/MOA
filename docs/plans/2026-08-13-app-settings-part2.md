...
- `RemoteError.detail`에 **만들 때의 언어가 박힌다** — 지금은 오류를 만들자마자 문자열로 굳혀 화면에 보내므로 문제가 드러나지 않지만, 오류를 구조체째 들고 매 프레임 다시 그리는 화면이 생기면 래퍼는 새 언어, 조각은 옛 언어로 갈린다. `Unsupported.operation`을 `RemoteOp`로 보존하면 표시 시점에 다시 풀 수 있다 (T7 quality 리뷰 m1).
