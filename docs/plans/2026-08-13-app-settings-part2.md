...
- **오류 사유가 언어 전환을 따라가지 않는다** — `RemoteError.detail`이 만들 때의 언어로 굳고, 그 문자열이 `ConnPhase::Failed` → `TabPhase::Error`에 남아 **매 프레임 다시 그려진다**. 연결 실패 화면을 띄운 채 언어를 바꾸면 제목·버튼은 새 언어인데 사유 줄만 옛 언어로 남아 한 줄에 두 언어가 섞이고(`ui/remote_states.rs:282`가 둘을 이어 붙인다), 전송 큐의 사유는 `Session.error: String`으로 저장돼 재시작 뒤에도 그렇다.
  - **처음 이 항목을 적을 때의 사유("지금은 문제가 드러나지 않는다")는 틀렸다** — Phase F가 실측으로 잡았다. 이 결함은 이번 작업이 원인이고 FR-53(Must)에 닿는다.
  - **고치지 않기로 했다**(2026-08-14 사용자 결정): 오류를 문자열이 아니라 구조로 들고 표시 시점에 문구를 만들어야 하는데 `ConnPhase::Failed`·`TabPhase::Error`·`TransferState::Error`가 10개 파일에 걸치고 세션 저장 형식(NFR-7)까지 바꾼다. 대신 **PRD FR-53에 한계를 적었다** — "다음 실패 때 새 언어로 나온다".
...
- [x] **T7. 문구 이관 ⑤ 원격 계층 오류·로그 문구 (65개, 동적 조립 중심)**
  - **Type**: D
...
- Phase F 통과 (F-1~F-7 — F-8은 `## 시각 요소 분해` 없음으로 N/A)
- Phase G 통과 (Must 100% — FR-53·NFR-6 + part1이 커버한 FR-13·FR-47~FR-52·NFR-7을 마지막 part 원칙대로 전체 트리 기준 재대조)
...
- T2 완료: 설정 화면에 `언어` 그룹. 고르면 그 프레임에 값이 바뀌고 `set_language`+`request_repaint`로 다음 프레임부터 화면이 바뀐다.
...
- T3 완료: 창 껍데기 68개 이관(배정표 50 + part1 신규분 18). 대상 11개 파일에 UI 한글 리터럴 **0개**가 남았다(위젯 ID 4곳 제외 — 번역 금지 목록).
  ...
  - **함정(리뷰 둘이 함께 잡음, BLOCKER)**: 일괄 치환이 **시험의 기대값까지 바꿔** `assert_eq!(i18n::connect(), i18n::connect())` 같은 항진명제를 만들었다. 빌드·테스트가 전부 초록인데 그 단언은 카탈로그가 무엇으로 바뀌어도 통과한다 — *"문구가 조용히 바뀌는 것"*을 막으라고 있던 가드가 바로 그 위험에 대해 죽었다. **문자열을 치환할 때는 `mod tests`의 기대값이 함께 바뀌지 않았는지 diff에서 확인한다.**

- T4 완료: 목록·패널·상태 줄 27개 이관(위젯 ID `원격 메뉴`와 `debug_assert` 메시지는 화면에 안 나와 대상 밖). 값이 끼어드는 문구 6개는 `i18n::dynamic`의 손수 쓴 함수로 갔다.
  - 상태 줄의 `"{}건 대기"` + `" 남음"` 조각 결합을 없앴다 — 개수와 남은 시간은 언어마다 붙는 자리가 달라(한국어 `남음`, 영어 `left`) 그 두 조각만 언어별로 만들고 사이의 속도·구분점은 순서가 같아 이어 붙인다.
  - **함정(spec 리뷰가 짚음)**: 조각을 옮기다 `i18n`이 `remote::queue::UNKNOWN`을 참조하게 만들었다 — **문구 카탈로그가 원격 계층을 거꾸로 참조**하는 것이다. 그 값은 호출부가 인자로 주도록 바꿨다.
  - **함정(quality 리뷰, T3에 이어 재발)**: 항진명제 단언이 `panel/tabs.rs`에 하나 더 생겼다(`assert_eq!(title(None), i18n::tabs_missing_site())` — 구현이 바로 그 함수를 부른다). 리터럴 비교로 되돌렸다.
  - **규약(quality 리뷰 M1)**: **프로덕션 코드가 카탈로그를 부르게 되면 그것을 단언하는 기존 시험도 언어를 잠가야 한다.** `workspace`·`create`·`status_bar`의 시험 9개가 잠금 없이 한국어 값을 단언하고 있었다 — 지금은 통과하지만 병렬 실행에서 다른 시험이 영어로 바꾼 순간을 만나면 깨진다.

- T5 완료: 사이트 관리자·원격 메뉴·원격 상태 87개 이관. 세 파일에 남은 한글은 위젯 ID 6곳뿐이다(plan 서문이 5곳이라 한 것은 오타 — 열거는 처음부터 6개였다. 함께 고쳤다).
  - **배열 상수는 함수가 됐다**: 카탈로그 값이 실행 중 언어를 읽으므로 `const [(Protocol, &str); 3]` 안에 넣을 수 없다. 프로토콜·암호화·로그온·전송 모드·문자셋 다섯이 그래서 `fn …_options()`가 됐고, 권한 대화의 `GROUPS`·`BITS`는 지역 `let`이 됐다.
  - 접근 키 알파벳(`프로토콜(T):` → `Protocol(T):`)은 영어에서도 그대로 뒀다 — 키 배정이 바뀌면 익힌 조작이 깨진다.
  - **반복 결함을 스크립트로 잡기 시작했다**(T3·T4에 이어 세 번째라): ① 단언 양쪽이 같은 카탈로그 함수를 부르는 **항진명제** ② 카탈로그 값을 한글 원문과 견주면서 **언어를 잠그지 않은 시험**. 각각 4건·7건을 찾아 고쳤고 지금은 둘 다 0건이다.
  - **리뷰가 잡은 것**: 배지 시험이 `badge_label`이 그대로 돌려주는 함수와 견주는 **간접 항진명제**였고(스크립트가 못 잡는 형태), 이관으로 가리킬 대상을 잃은 섹션 헤더 주석과 접근 키 설명이 접근 키 없는 첫 항목에 붙은 doc이 있었다.

- T6 완료: 앱 본체·전송 큐·도크 39개 이관. 값이 끼어드는 문구 7개가 `dynamic`으로 갔고 두 언어 모두 리터럴로 고정하는 시험을 붙였다.
  - **탐색 규칙을 바로잡았다(리뷰가 BLOCKER로 잡음)**: 항진명제를 "같은 함수가 양쪽에" 로 찾았더니 `assert_eq!(headers(), [i18n::a(), i18n::b(), …])`처럼 **각기 다른 함수를 늘어놓은 형태**를 놓쳤다. 규칙을 **"단언의 기대값에 카탈로그 호출이 있으면 의심"**으로 바꾸니 6건이 더 나왔다(`queue_panel` 4·`site_manager` 1·`view_mode` 1 — `view_mode`는 T3에서 이관한 것이라 두 회차를 지나 있었다).
    - 규약: **기대값은 언제나 원문 리터럴**이다. 좌변이 그 함수를 그대로 돌려주든 아니든, 기대값에 카탈로그를 쓰면 값이 무엇으로 바뀌어도 통과한다.
  - 배열 상수 `HEADERS`는 `fn headers()`가 됐다(T5의 다섯 배열과 같은 이유).
  - 정리: 쓰지 않는 카탈로그 키 3개를 뺐다 — 작업 이름이 메뉴 문구와 같은 말이라 그 키를 그대로 쓴다.

- T7 완료: 원격 계층 65개 이관. `RemoteError`의 여덟 문장이 언어별로 갈리고, 동사 조각은 **언어 중립 `RemoteOp`**(22 variant + `Raw`)가 나른다 — FTP 프로토콜 명령어(`LIST`·`CWD`…)는 `Raw`로 감싸 번역하지 않는다.
  - **로직이 언어를 따라가 버린 사고(spec·quality 둘 다 BLOCKER/MAJOR)**: 일괄 치환이 `ftp.rs`의 `mentions_permission`까지 바꿔, **서버 응답을 살피는 휴리스틱이 화면 언어를 따르게** 됐다. 앱을 영어로 두면 한국어로 답하는 서버의 `권한` 문구를 못 잡아 `PermissionDenied`가 `NotFound`로 오분류된다 — 문구가 아니라 **분기가 틀어지는** 결함이라 리터럴로 되돌렸다.
    - 규약: **서버·파일시스템에서 온 문자열을 살피는 코드는 카탈로그를 부르지 않는다.** 화면에 보일 말과 남의 말을 견주는 잣대는 다른 것이다.
  - **항진명제 탐색 규칙을 세 번째로 고쳤다**(T3→T6→여기): `assert!(v.contains(&i18n::x()))` 형태가 또 빠져나갔다. 최종 규칙은 **"단언 안에 카탈로그 호출이 있는데 한글 리터럴이 하나도 없으면 의심"** — 형태를 따지지 않으므로 `contains`·`starts_with`·배열 비교가 모두 걸린다. 11건이 나왔다.
  - 한계(기록): 이 두 스크립트는 **개발 중 손으로 돌리는 것**이라 T8의 회귀 테스트로 옮길지는 그 task에서 정한다.

- T8 완료: 소스 훑기 회귀 테스트·규약 기록·문서.
  - **테스트가 곧바로 제 몫을 했다**: 넣자마자 이관에서 빠진 문구 2건(호스트 키 지문 경고 — `remote_states`·`sftp`)을 잡았다. 여러 줄 이어붙이기(`\`)라 손으로 훑을 때 놓쳤던 것이다.
  - **그 이관이 새 결함을 만들었고 리뷰 둘이 함께 잡았다**: 카탈로그로 옮기며 `\`를 빠뜨려 **문장 가운데 공백 16칸**이 박혔다. 빌드·테스트·clippy·fmt 어느 것도 문자열 내용을 보지 않는다 — 두 함수의 반환값을 리터럴로 단언하는 시험을 붙여 막았다.
  - 예외 목록: 파일 단위 5개(레거시 Win32 4 + `panel/file_list.rs`)와 리터럴 단위 17개. `remote/testing.rs`는 한글이 셋뿐이라 **리터럴 단위로 내렸다**(spec 리뷰 M1 — "리터럴로 하나씩 적을 수 없는 것만"이라는 자기 기준에 어긋났다).
  - 한계(테스트 doc에 적음): 리터럴만 본다 — 문구를 변수에 담아 옮기거나 바깥에서 받아 그리면 걸리지 않는다.

- **Phase F·G 통과.** 자동 검증 전부 통과(빌드 경고 0 · 테스트 694건 · clippy `-D warnings` · fmt). F-7이 BLOCKER 1·MINOR 3을 냈고 전부 닫았다.
  - **BLOCKER가 잡은 것은 내가 이연하며 적은 사유가 틀렸다는 것이었다** — "지금은 문제가 드러나지 않는다"고 했으나, 굳은 오류 사유가 `TabPhase::Error`에 남아 **매 프레임 다시 그려지므로** 연결 실패 화면에서 한 줄에 두 언어가 섞인다. 전송 큐의 것은 세션에 저장돼 재시작 뒤에도 그렇다.
    - 사용자 결정(2026-08-14): **고치지 않고 한계를 적는다**(선택 C). 고치려면 오류를 구조로 들어야 하는데 10개 파일과 세션 저장 형식(NFR-7)에 걸친다. PRD FR-53·README·plan·대장에 한계를 적었다.
    - **규약**: 이연할 때 적는 사유는 **그 자체가 검증 대상**이다. 근거 없이 "드러나지 않는다"고 적으면 그 판단이 다음 세션까지 사실로 굳는다.
  - 배정표 대조(F-7 독립 측정): base의 한글 출현 276건 → 고유 239건(중복 37이 한 키를 공유) → 카탈로그 216 + 동적 재구성 16 + 비대상 7. **조용히 사라진 화면 문구 0건**, 한국어 값은 원문 그대로.
  - MINOR 3건 처리: 죽은 카탈로그 키 1개 제거, 회귀 테스트의 단언 건너뛰기 창을 세 줄→같은 줄로 좁힘(넓은 창은 단언 옆 화면 문구를 통과시킨다), 검사가 못 보는 자리 셋을 테스트 설명에 기재.
  - **⏳ HUMAN-VERIFY (수동 검증 ①~⑤ — 미수행)**: 화면을 띄워야 아는 것들이다. 특히 ②(영어로 바꿔 **잘리거나 겹치는 곳** 찾기 — 사이트 관리자 3탭·전송 큐 열 머리글·상태 표시줄·연결 실패 화면이 폭이 빡빡하다. 전제 8이 "성립을 좌우하지는 않는다"로 남겨 둔 미확인 항목이다).
