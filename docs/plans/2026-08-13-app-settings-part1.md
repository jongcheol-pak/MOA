...
| `app::fonts::FontCatalog`(`scan`·`korean_names`·`load`) / `load_font`(+`LoadedFont`) | `install_fonts`가 경로 상수 하나를 읽을 뿐 열거·추출 기능 없음 | **신규** — 기존에 없다 |
...
- [x] **T4. 시스템 글꼴 열거와 글꼴 바이트 읽기**
  - **Type**: C
  - **Design**: ① `src/app/fonts.rs` 신규 — Win32 GDI를 쓰지만 파일시스템 열거(`fs`)도 UI(`ui`)도 아닌 "앱 환경 조회"라 `app`에 둔다(`app::theme`이 `DwmSetWindowAttribute`를 쓰는 것과 같은 자리). ② 신규 심볼 — `FontCatalog::korean_names() -> Vec<String>`(한글 지원 글꼴 이름을 가나다순 중복 없이), `load_font(name: &str) -> Option<Vec<u8>>`. ③ `ui`가 이것을 부르고, `fonts`는 아무것도 참조하지 않는다. **`FontCatalog::korean_names()`는 1.5초쯤 걸린다**(실측: 이름 열거 1ms + 93개 전수 읽기 1,525ms) — 이름만 세지 않고 실제로 읽어 확인하기 때문이다. 그래서 **UI 스레드에서 부르면 안 되고**, T5가 워커에서 부른다(AGENTS "UI 스레드에서 블로킹 I/O 금지"). ④ 이번에 추상화하지 않을 것: 글꼴 캐시·메타데이터 구조체를 두지 않는다(이름과 바이트면 충분하다).
  ...
  - **Acceptance**: Given 한국어 Windows, When `FontCatalog::korean_names()`를 부르면, Then 결과에 `맑은 고딕`과 **글꼴 모음에 든 `굴림`·`바탕`이 함께** 있고 한글 글리프가 없는 `Wingdings`·`Webdings`는 없다. **목록의 모든 이름에 대해** `load_font(name)`이 `Some`이다(바이트를 얻을 수 있다). 목록에 없는 이름(`load_font("없는글꼴")`)은 **GDI가 굴림으로 조용히 대체하더라도** `None`을 반환한다(실측: `없는글꼴이름XYZ` → 실제 선택 `굴림`, 데이터 크기까지 굴림과 동일).
...
- [x] **T10. 자동 실행으로 시작할 때 트레이로만 올라오기**
  - **Type**: B
...
- 서식: `cargo fmt --check`
- 수동 검증 (T13에서 한 번에): ① 설정 열기 → 네 그룹 표시(`언어`는 part2) ② 글꼴 교체 즉시 반영·재시작 유지 ③ 자동 실행 토글 후 `regedit`으로 Run 키 확인 ④ 트레이 토글 on → **아이콘이 뒤집히지 않고 정상으로 보이는지**(코드로는 확인 불가 — GDI 비트맵 방향) → `✕` → 숨김 → 더블클릭 복원 → 우클릭 메뉴 `종료` → 탐색기를 작업 관리자에서 다시 시작한 뒤 아이콘이 되살아나는지 ⑤ 트레이 상태에서 exe 재실행 → 창이 하나만 ⑥ 확장자·숨김 토글을 로컬·원격 패널에서 각각 확인 ⑦ NFR-1·NFR-2 측정

## Phase Ledger

- Phase F 통과 (F-1~F-7 — F-8은 `## 시각 요소 분해` 없음으로 N/A)
- Phase G 통과 (Must 100% — FR-13·FR-47~FR-52·NFR-7. FR-53·NFR-6은 part2 몫)
...
- T7-T9 완료 (커밋 `6a97b17`·`c6ba5e2`·`4f1c43f`): 트레이 아이콘·닫기 가로채기·중복 실행 방지.
  - **핵심 설계(D5)가 실측으로 정해졌다**: 창을 되살리는 일은 앱 프레임이 아니라 **창 프로시저가 직접** 한다. eframe은 창이 숨겨져 있으면 UI 콜백을 건너뛸 수 있어, 채널로 보내고 프레임에서 띄우는 방식은 그 프레임이 오지 않으면 창을 영영 되살릴 수 없다.
  - 함정: 숨긴 동안 `track_window`를 멈추지 않으면 숨은 창의 값이 저장해 둔 좋은 값을 밀어낸다.
  - 함정: 복원 뒤 `Visible(true)`를 보내지 않으면 winit 가시성 캐시가 `false`로 남아, 이후 최대화 등 창 플래그가 바뀔 때 `apply_diff`가 `SW_HIDE`를 다시 적용해 창이 갑자기 사라진다.
  - **규약(리뷰가 잡음)**: 함수를 끼워 넣을 때 **인접 독스트링이 딸려 붙지 않았는지** 본다. `intercept_close`를 삽입하며 `show_settings_dialog`의 독스트링이 그쪽으로 붙었다.
  - **규약(리뷰가 잡음)**: 시스템 자원(레지스트리·뮤텍스)을 만지는 시험은 **프로덕션과 다른 이름**을 쓴다. 같은 이름을 쓰면 개발 중 앱을 띄워 둔 채 테스트할 때 깨진다.
  - 규약: 비트맵을 GDI에 넘길 때 행 방향을 손으로 뒤집지 말고 `biHeight` 음수로 **명시**한다(`ui::icon_tex`와 같은 방법) — 방향을 밝히지 않는 API에 넘기면 어느 쪽이 맞는지 코드로 알 수 없다.

- T11 완료: 파일 확장명 표시 토글. `ListRow::display_name(show_extensions)`을 트레이트 기본 구현으로 두어 로컬·원격이 같은 규칙을 쓴다.
  - 설계(D7 준수): **그리는 6곳만** 이 메서드를 쓰고 경로 조립·정렬 키·선택 복원은 원본 `name()` 그대로다. 배선은 `app` → `splitter` → `PanelState::set_show_extensions` → `FileListView`로 매 프레임 내려 준다.
  - **규약(quality 리뷰 M3이 잡음)**: 다음 task에 쓸 값을 미리 실어 두지 않는다. `DisplayRules { show_extensions, show_hidden }`으로 묶었다가 `show_hidden`을 아무도 읽지 않는 죽은 필드로 지적받아 `bool` 하나로 되돌렸다 — T12가 필요해지면 그때 묶는다.
  - **규약(리뷰가 또 잡음, T8에 이어 두 번째)**: 함수를 끼워 넣을 때 **인접 독스트링이 딸려 붙지 않았는지** 본다. 이번엔 `PanelState::show`와 `FileListView::view_mode` 둘이 자기 설명을 잃었다. 삽입 지점 앞뒤로 빈 줄이 있는지가 판정선이다.
...
- **Phase F·G 통과.** 자동 검증은 전부 통과했다(빌드 경고 0 · 테스트 672건 · clippy `-D warnings` · fmt). F-7이 MAJOR 3·MINOR 7을 냈고 코드·문서 쪽은 모두 닫았다(커밋 `5179abb`) — 남은 하나는 사람만 할 수 있는 확인이다.
  - **F-7이 잡은 실질**: 폴더 트리가 숨김 설정을 따르지 않아 **같은 창의 목록과 트리가 다르게 보였다**. T12의 Acceptance가 "목록"만 말해 범위 밖으로 볼 여지가 있었으나, 사용자 눈에는 설정이 반만 듣는 것이라 고쳤다.
  - **⏳ HUMAN-VERIFY (수동 검증 ①~⑥ — 미수행)**: GUI·트레이·부팅 동작은 코드로 확인할 수 없다. PRD도 FR-48·FR-49·FR-50을 HUMAN-VERIFY로 규정한다. 특히 ⑤(트레이 상태에서 exe 재실행 → 기존 창이 **최상위로** 올라오는지)는 `SetForegroundWindow`가 포그라운드 자격 없이는 작업 표시줄 깜빡임으로 끝날 수 있어 실환경 확인이 필요하다. ⑦(NFR 측정)만 T13에서 수행했다.
  - 관측(재현 실패): `cargo test --lib`가 **소스 수정 직후 첫 실행**에서 두 차례 1건 실패했으나 어느 것인지 잡지 못했고, 이후 각각 5회·5회 연속 전건 통과했다. 실패 테스트 이름을 남기지 못해 원인 미상으로 둔다.
