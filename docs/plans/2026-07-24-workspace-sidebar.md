...
## Phase Ledger
- 전 task 완료 (T1~T9, 마지막 완료 커밋 6ddd62d)
- Phase F 통과 (HEAD 6ddd62d) — F-7 1차에서 BLOCKER 1(B1: 부제 갱신이 인라인 편집 취소) 발견 → T9 추가·수정 후 F-2 전체 테스트(75+2) + F-7 재호출 통과(BLOCKER 0·MAJOR 0·MINOR 1 → 문서 보완 완료)
- Phase G 통과 (Must 100%) — G-1은 F-7 2차의 PRD 전수 대조 재사용(incomplete·529 대체 없음): active Must FR-1~8·FR-15~17 충족, Should FR-9~12·FR-18~20 충족, NFR-1~3·NFR-8 실측 충족, NFR-4~7 회귀 없음. ❌ 0건, 재루프 0회. 시각·조작 체감은 F-8 HUMAN-VERIFY 잔여
- F-8 보류 — `### V-9 대조 결과`의 렌더 행이 전부 `⏳ 미확인`(데스크톱 UI 자동 캡처 불가) → 완료 선언 보류, 사용자 화면 확인 대기
...
- T7-T8 완료 (커밋 a3ea7d2, T8): ① 사이드바 접기(상단 ◧ 버튼·보기 메뉴·Ctrl+B)·경계 드래그 폭 조절(160~480)·폭/접힘 세션 저장, 배치 계산을 순수 함수 4종으로 분리. ② 성능 실측·지연 생성 실동작 검증·전체 회귀.
  - **실측(release 439KB, APPDATA 리다이렉트)**: NFR-1 시작 중앙값 **258ms**(3회 664/258/251, <1s) · NFR-2 1워크스페이스·2패널 유휴 **23.6MB**(<50MB) · NFR-8 워크스페이스 5개 방문 유휴 **24.1MB**(<100MB, 1개 방문 23.3MB 대비 +0.8MB).
  - **지연 생성 교차 검증**: 5개 워크스페이스 세션으로 시작 시 패널 창 1개·스레드 5 → 5개 방문 후 패널 창 5개·스레드 9 → 4개 삭제 후 패널 창 1개·스레드 5로 완전 회수(누수 없음). 미방문 워크스페이스 데이터도 이름 그대로 재저장 확인.
  - 시각 요소 분해 표 ↔ 코드 상수 전 행 1:1 일치 재확인(색 10종·치수 15종·폭 토큰 3종·경계 4px). **렌더 일치는 여전히 `⏳ 미확인` — F-8에서 사용자 확인 필요.**
- T9 완료 (커밋 6ddd62d, F-7 BLOCKER B1 대응): 부제 자동 갱신이 인라인 편집을 취소하던 결함 수정 — `Sidebar::refresh_items`(편집 보존, 스냅숏만 교체)를 신설해 `set_items`(구조 변경 경로 — 편집 종료 유지)와 분리하고 `refresh_subtitle`만 새 API를 쓴다.
  - 결정: 편집 보존 여부를 플래그가 아니라 **두 개의 함수**로 나눈다 — 호출부의 의미(구조 변경 vs 표시 갱신)가 다르고, 플래그는 호출부마다 판단을 미루게 되어 같은 실수가 재발한다.
  - 근본 원인: 새 워크스페이스의 첫 폴더 열거가 끝나면 패널이 `WM_APP_PATH_CHANGED`를 **게시**하므로, 생성 직후 연 편집이 항상 그 뒤에 도착하는 알림에 의해 결정적으로 취소됐다(타이밍 레이스가 아님).
  - 검증: release 실행 후 `WM_APP_WS_NEW` 게시 → 사이드바 자식 EDIT 폴링 — 생성 직후 '워크스페이스 2', 첫 열거 완료 3초 후에도 유지 확인. 부수 효과로 편집 중 패널 클릭 시에도 입력이 커밋된다(WM_KILLFOCUS 경로).

## Next Steps
- 권장 다음 액션: **사용자 화면 확인(F-8)** — `cargo run`으로 실행해 사이드바 렌더·조작을 확인. 어긋난 항목을 알려주면 그 항목만 Phase P/I로 재진입해 수정한다
- 확인 후 승인 시: master 병합·push는 별도 승인 필요 (자율 루프 권한 밖)
- Suggested skills: pjc:implement-task(수정 재진입 시), 공식 /code-review
