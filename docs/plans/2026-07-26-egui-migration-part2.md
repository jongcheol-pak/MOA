...
- **워크스페이스 Delete 키 배정** — 현행 Win32 판은 사이드바에 포커스가 있을 때 Delete로 삭제한다. egui에는 "사이드바에 포커스가 있을 때만"이라는 구분이 없어 전역으로 받으면 **파일 목록에서 누른 Delete까지** 워크스페이스를 지운다. 그래서 이번엔 키를 배정하지 않고 메뉴 표기도 지웠다(F-7 M2). 영역별 포커스 개념을 도입하면 되살릴 수 있다
