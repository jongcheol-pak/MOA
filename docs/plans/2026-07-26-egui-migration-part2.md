...
> **달성 상태 (2026-07-28)**: 앞 문장(기능적 대체)은 달성했다. 뒷 문장(Win32 코드 제거)은 **T7에서 사용자가 보류를 선택**해 미달성이며 Deferred로 넘겼다 — 그 코드는 소스에 남아 컴파일만 되고 실행 파일에서는 쓰이지 않는다.
...
- **Edge Cases**: 저장된 경로가 사라짐(외장 드라이브) → ~~홈 폴더 폴백~~ **구현하지 않음**: pending-커밋 모델이 이미 "찾을 수 없습니다"를 표시하고 이전 위치를 유지하며, 현행 Win32 판도 같다. 임의로 홈으로 보내면 사용자가 무엇이 사라졌는지 모른다 / 저장된 창 위치가 화면 밖(모니터 구성 변경) → 화면 안으로 보정 / 최대화 상태 저장 / 워크스페이스 0개인 세션 파일 → 기본 1개 생성 / `%APPDATA%` 쓰기 실패 → 조용히 무시(앱은 정상 종료) / 종료 중 저장과 감시 스레드 정리 순서
...
## T6 실측 결과 (release, 2026-07-28)

측정 환경: Windows 11 Pro 26300 · release 빌드 · 각 항목 3~4회 실행 중앙값. 메모리는 창이 뜬 뒤
안정화(3~9초)한 시점의 프로세스 값이며, 매 실행 전 `settings.json`을 지워 조건을 통일했다.

| 항목 | egui 판 | 현행 Win32 판 | 측정 방법 |
|---|---|---|---|
| exe 크기 | **4.56 MB** | 0.42 MB | `target/release` 파일 크기 (part1 3.87MB → 트리·사이드바·메뉴·세션 추가분) |
| 콜드 스타트 (2회차 이후) | **110~146 ms** (중앙값 133) | 158~272 ms (중앙값 253) | 프로세스 시작 → `MainWindowHandle` 등장까지 폴링 |
| 콜드 스타트 (첫 실행) | 278~975 ms | 540~585 ms | 위와 같음. 첫 실행은 셰이더·폰트 캐시가 섞인다 |
| 유휴 메모리 — 패널 1개 | Working Set **128.4 MB** / Private 90.6 MB | WS 21.4 MB / Private 4.9 MB | 홈 폴더, 워크스페이스 1개 |
| 유휴 메모리 — 패널 2개 | Working Set **137.6 MB** / Private 88.1 MB | WS 23.6 MB (2026-07-24 실측, README) | 2패널 세션을 만들어 복원시킨 뒤 측정 |
| 워크스페이스 5개 모두 방문 | Working Set **124.1 MB** / Private 88.5 MB | WS 24.1 MB (2026-07-24 실측) | 5개 생성·전부 방문 상태로 시작하는 임시 측정 빌드(측정 후 되돌림) |
| 워크스페이스 13개 모두 방문 | Working Set 133.2 MB | — | 위 측정을 세션 누적으로 반복 — 워크스페이스당 약 +0.8 MB |
| 10만 파일 폴더 | Working Set **143.8 MB** / Private 108.6 MB, 창 표시 100~169 ms | 자동 측정 불가 | egui 판은 폴더를 인자로 받는다. **현행 판은 시작 폴더 인자를 지원하지 않아** 같은 방식으로 잴 수 없다 |
| 10만 파일 렌더 p95 | 0.63 ms (part1 실측) | UI 응답 유지 (2026-07-23) | part1 이후 목록 렌더 경로는 바뀌지 않았다 — 재계측 대신 그 값을 인용 |

### NFR 판정

| NFR | 기준 | 실측 | 판정 |
|---|---|---|---|
| NFR-1 콜드 스타트 | 1초 이내 | 133 ms (2회차 이후), 첫 실행 최대 975 ms | ✅ 충족 — **첫 실행은 여유 25ms뿐**(셰이더·폰트 캐시가 빈 상태). 다른 환경에서는 넘길 수 있다 |
| NFR-2 유휴 메모리 | 150 MB 이하 | **137.6 MB** (패널 2개 기준) | ✅ 충족 — 여유 12.4 MB |
| NFR-3 10만 파일 | 무정지 | 렌더 p95 0.63 ms, 진입 후 143.8 MB | ✅ 충족 (조작 감각은 ⏳) |
| NFR-4 DPI | 배율 간 이동 | — | ⏳ HUMAN-VERIFY |
| NFR-5 긴 경로·유니코드 | 260자 초과·유니코드 표시 | **306자 경로 열거 성공**(회귀 테스트 추가), 유니코드 이름 보존 테스트 통과 | ✅ 열거 확인 / 화면 표시는 ⏳ |
| NFR-8 워크스페이스 5개 | 200 MB 이하 | **124.1 MB** | ✅ 충족 — 여유 76 MB |

**D10 게이트: 통과.** NFR-2·NFR-8 모두 기준 안이므로 T7(구 Win32 코드 제거)로 진행한다.

### 측정에서 드러난 사실

- **워크스페이스는 거의 공짜다** — 5개(124.1MB)와 1개(128.4MB)가 측정 오차 안에서 같고, 13개까지 늘려도 133.2MB다. 워크스페이스당 약 0.8MB — 지연 생성(D1)과 아이콘 캐시 전역 공유가 함께 작동한 결과다. NFR-8의 200MB는 크게 남는다.
- **메모리의 바닥은 워크스페이스가 아니라 egui 자체**다(폰트·GL 컨텍스트). 더 줄이려면 한글 폰트 서브셋이 유일한 큰 후보다(Deferred).
- **세션 복원이 실측으로 증명됐다** — 측정 중 워크스페이스가 실행마다 누적 복원돼 13개가 됐고(이름·탭 경로까지 정확), 2패널 분할도 세션으로 복원해 측정에 썼다. FR-11·FR-20의 저장·복원 경로가 실제로 도는 것을 우연히 확인한 셈이다.
- **자동 입력 주입은 여전히 안 먹는다** — `SendKeys`로 Ctrl+\를 보내도 분할이 일어나지 않았다(part1의 마우스 주입 한계와 같은 현상). 그래서 패널 2개·워크스페이스 5개 상태는 **세션 파일과 임시 측정 빌드로** 만들었다.
...
- **워크스페이스 Delete 키 배정** — 현행 Win32 판은 사이드바에 포커스가 있을 때 Delete로 삭제한다. 지금 구조에서는 사이드바가 키를 전역으로 보기 때문에, 전역으로 받으면 **파일 목록에서 누른 Delete까지** 워크스페이스를 지운다. egui에 위젯 포커스 API(`has_focus`)는 있으므로 카드에 포커스를 주면 해결되지만, 그 전환은 사이드바 입력 전반에 걸친 작업이다. 그래서 이번엔 키를 배정하지 않고 메뉴 표기도 지웠다(F-7 M2). 영역별 포커스 개념을 도입하면 되살릴 수 있다
- **F2도 같은 성질의 전역 키다** — 파일 목록에서 F2를 눌러도 워크스페이스 이름 편집이 열린다. 파괴적이지 않고 Esc로 취소되어 이번엔 두었으나, 위 포커스 개념을 도입할 때 함께 정리한다
- **워크스페이스 키보드 위/아래 전환** — 현행 판에 있던 것이 이식에서 빠졌다. PRD·plan 요구는 아니다 (F-7 참고 지적)
- **T6 측정 재현 절차** — 워크스페이스 5개 수치는 임시 측정 빌드로 얻었고 측정 스크립트는 레포에 남기지 않았다. 다시 재려면 같은 패치를 만들어야 한다 (F-7 m5)
