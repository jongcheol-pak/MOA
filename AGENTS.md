...
- **비밀번호**: `%APPDATA%` 파일에 **DPAPI로 봉인해서만** 담는다 (`remote::secret`). 평문을 파일·로그·문서에 남기지 않는다
  - **통로가 하나 더 있다 — 사이트 목록 내보내기 파일**(`remote::envelope`, FR-59): 그 파일은 다른 PC로 옮기려고 만드는 것이라 DPAPI(사용자·PC에 묶인다)로는 뜻이 없어, 사용자가 정한 암호에서 키를 파생해 봉한다(CNG PBKDF2-HMAC-SHA256 + AES-256-GCM). **그 문서에 `password_sealed`(DPAPI 바이트)를 담지 않는다** — 담으면 같은 PC에서 「비밀번호 없이 내보낸 파일」의 비밀번호가 되살아난다
...
- UI 스레드에서 파일시스템 블로킹 호출 — 겨냥하는 것은 **매 프레임 도는 렌더·탐색 경로**다(디렉터리 열거·감시). 사용자가 직접 누른 드문 조작의 작은 파일 I/O는 예외이며 지금 둘뿐이다: 세션 저장(`persist_session`)과 사이트 목록 내보내기·가져오기(`ui::app::pump_site_file_dialog` — 키 파생 0.126초 실측을 포함해 UI 스레드에서 돈다)
