...
pub struct ExportedSite {
    ...
    pub logon: LogonType,
    pub user: String,
    /// 개인 키 파일 경로 (FR-66) — **키 암호는 담지 않는다**(D6).
    ///
    /// 암호를 실으려면 아래 비밀번호 평행 배열의 형식을 바꿔야 하고, 그러면 직전 버전이
    /// 만든 파일과 호환이 깨진다. 경로만 옮기고 암호는 가져온 뒤 한 번 입력받는다.
    /// `#[serde(default)]`라 이 필드를 모르는 직전 버전의 파일도 그대로 읽힌다
    #[serde(default)]
...
fn write_record(store: &mut SiteStore, id: SiteId, site: &ExportedSite) {
    ...
    let kept_key = store
        .get(id)
        .map(|record| record.key_passphrase_sealed.clone())
        .unwrap_or_default();
    store.insert(SiteRecord {
        ...
        // 키 암호는 문서에 없다(D6) — 있던 것을 지우지 않고 그대로 둔다
        key_passphrase_sealed: kept_key,
...
fn effective_user(site: &ExportedSite) -> &str {
    match site.logon {
        ...
        LogonType::Normal | LogonType::KeyFile => &site.user,
    }
}
...
mod tests {
    ...
    #[test]
    fn 키_경로는_담기고_키_암호는_담기지_않는다() {
        // D6 — 암호를 실으려면 비밀번호 평행 배열의 형식을 바꿔야 해 직전 버전과 호환이 깨진다.
        // 경로만 옮기고 암호는 가져온 뒤 사용자가 한 번 입력한다
        let mut source = SiteStore::new();
        let id = add_site(&mut source, "키 서버", "key.test", "deploy");
        if let Some(record) = source.get_mut(id) {
            record.logon = LogonType::KeyFile;
            record.key_path = Some(PathBuf::from(r"C:\keys\id_ed25519"));
        }
        assert!(source.set_key_passphrase(id, "옮겨가면안되는키암호"));

        let outcome = build(&source).expect("내보내기");
        let text = serde_json::to_string_pretty(&outcome.document).expect("직렬화");
        assert!(
            !text.contains("옮겨가면안되는키암호"),
            "키 암호가 문서에 실렸다"
        );
        ...
        let copied = target.sites().first().expect("옮겨온 사이트");
        assert_eq!(
            copied.logon,
            LogonType::KeyFile,
            "로그온 유형이 함께 옮겨간다"
        ...
        assert_eq!(
            copied.key_path,
            Some(PathBuf::from(r"C:\keys\id_ed25519")),
            "키 경로는 옮겨가야 한다"
        );
        assert_eq!(
            target.key_passphrase(copied.id),
            None,
            "키 암호는 옮겨가지 않는다 — 가져온 뒤 한 번 입력받는다"
        );
    }

    #[test]
    fn 키_경로를_모르는_직전_버전의_파일도_그대로_읽힌다() {
        // `#[serde(default)]`라 그 필드가 없는 문서도 읽힌다 — 있던 호환이 깨지지 않는다
        let mut source = SiteStore::new();
        add_site(&mut source, "옛 서버", "old.test", "deploy");
        let outcome = build(&source).expect("내보내기");
        let mut text = serde_json::to_string(&outcome.document).expect("직렬화");
        // 직전 버전이 만든 문서에는 이 키가 아예 없다
        text = text.replace("\"key_path\":null,", "");
        assert!(!text.contains("key_path"), "시험 준비가 어긋났다: {text}");

        let document = parse(&text).expect("직전 버전 문서를 해석하지 못했다");
        let mut target = SiteStore::new();
        let plan = plan_import(&document, &target, "").expect("계획");
        apply_import(&mut target, &plan, true);
        let copied = target.sites().first().expect("옮겨온 사이트");
        assert_eq!(copied.key_path, None);
    }
