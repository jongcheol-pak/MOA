...
//!
//! **`decode_name`·`encode_name`을 아직 아무도 부르지 않는다** (미완성이 아니라 막힌 것):
//! `suppaftp`가 목록 줄을 `String::from_utf8_lossy`로 이미 옮겨 내주고(`sync_ftp.rs:747`·`:841`)
//! `ssh2`는 경로를 `from_utf8().unwrap()`으로 옮겨, **원본 바이트가 우리 손에 오는 지점이 없다.**
//! 잇는 길(저수준 데이터 스트림 직접 파싱 · SFTP 크레이트 교체)은 plan `## Deferred / Follow-up`의
//! 「원격 목록에 문자셋을 실제로 적용하기」에 적어 두었다.
