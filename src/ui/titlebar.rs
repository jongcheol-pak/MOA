...
pub struct UpdateBadge {
    ...
    /// 받는 중일 때 지금까지 받은 비율 — 서버가 전체 크기를 알려 주지 않으면 `None`이고,
    /// 그때는 백분율 없이 종전 문구만 보인다 (FR-62·D1). 받는 중이 아니면 뜻이 없다
...
fn badge_parts(badge: UpdateBadge) -> (&'static str, String) {
    if badge.downloading {
        let label = match badge.percent {
            Some(percent) => crate::i18n::dynamic::update_downloading_percent(percent),
            // 전체 크기를 모르면 보일 비율이 없다 — 종전 문구 그대로다 (D1)
            None => crate::i18n::update_downloading().to_owned(),
        };
        (egui_phosphor::regular::CIRCLE_NOTCH, label)
    } else {
        (
            egui_phosphor::regular::ARROW_CIRCLE_DOWN,
            crate::i18n::update_available().to_owned(),
        )
    }
}
...
mod tests {
    ...
    fn 받는_중에는_문구가_갈리고_눌리지_않는다() {
        ...
        // 전체 크기를 알면 그 자리에 백분율이 든다 (T3 acceptance ⓐ·ⓑ)
        let with_percent = UpdateBadge {
            percent: Some(42),
            ..downloading
        };
        assert_eq!(
            badge_parts(with_percent).1,
            crate::i18n::dynamic::update_downloading_percent(42)
        );
        assert_eq!(
            badge_parts(with_percent).0,
            badge_parts(downloading).0,
            "아이콘은 그대로다 — 갈리는 것은 글자뿐이다"
        );
        assert_ne!(
            badge_parts(idle).0,
            badge_parts(downloading).0,
