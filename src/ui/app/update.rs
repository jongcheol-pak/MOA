...
impl ExplorerApp {
    ...
    pub(super) fn update_badge(&self) -> titlebar::UpdateBadge {
        ...
        match self.update.status() {
            ...
            UpdateStatus::Downloading { received, total } => titlebar::UpdateBadge {
                visible: true,
                downloading: true,
                // 서버가 전체 크기를 알려 주지 않으면 `None`이 되고 배지는 종전 문구를 쓴다 (D1)
                percent: crate::app::update::download_percent(*received, *total),
                update_enabled: enabled,
            },
