...
// ── 끌어 정렬 공용 조각 — 목록을 끌어 차례를 바꾸는 화면들이 함께 쓴다 ──
/// 클릭을 재정렬로 보기 시작하는 이동 거리.
///
/// 이보다 덜 움직이면 그 자리의 클릭(폴더 이동·선택)으로 처리한다. 끌어 정렬을 쓰는
/// 화면이 같은 값을 써야 「끌어서 옮긴다」가 자리마다 다른 손맛이 되지 않는다
...
///
/// **좌표를 다루지 않는 순수 인덱스 산술**이라 좌표계가 다른 화면들이 그대로 함께 쓴다.
...
pub(crate) fn menu_row(ui: &mut egui::Ui, label: &str, enabled: bool) -> bool {
    ...
    // **누름만 돌려준다** — 이 줄을 쓰는 메뉴(원격·트리·드롭다운)에는 하위 메뉴가 없어
    // 마우스가 얹혔는지는 부르는 쪽이 쓸 데가 없다
    menu_row_rich(ui, MenuRowIcon::None, label, "", false, enabled).clicked
}

/// 메뉴 줄 왼쪽에 놓일 그림 — 두 갈래다.
///
/// 셸이 준 항목은 **비트맵**(텍스처로 올려 둔 것)이고, 우리가 넣는 줄(`기본 메뉴`·`앱 확장`)은
...
/// 메뉴 줄 하나에서 이번 프레임에 일어난 일 (FR-8 재개정).
///
/// **누름과 얹힘을 함께 돌려준다** — 하위 메뉴가 있는 줄은 **마우스를 올리기만 해도** 펼쳐야
/// 하는데(2026-08-26 사용자 요청, 이 앱의 패널 메뉴가 이미 그렇다), 누름만 돌려주면 그리는
/// 쪽이 그 신호를 얻을 길이 없다.
///
/// **`hovered`는 비활성 줄에서도 참이 될 수 있다** — 그 줄은 `Sense::hover()`로 잡히기
/// 때문이다. 반면 `clicked`는 활성일 때만 참이다(눌러도 되지 않는 것이 눌리면 안 된다)
///
/// **`Eq`를 파생하지 않는다** — `top`이 `f32`라 그 트레이트를 만족하지 않는다.
/// `PartialEq`는 시험이 값을 견주는 데 쓰므로 남긴다
#[derive(Debug, Clone, Copy, PartialEq)]
pub(crate) struct MenuRowHit {
    /// 이번 프레임에 눌렸는가 — 비활성 줄은 언제나 거짓이다
    pub clicked: bool,
    /// 마우스가 얹혀 있는가
    pub hovered: bool,
    /// 이 줄의 **위쪽 화면 좌표** — 하위 메뉴를 그 높이에 띄우는 데 쓴다 (2026-08-26).
    ///
    /// 종전에는 하위 팝업이 부모 메뉴의 맨 위에 붙어, 어느 줄을 펼쳤든 같은 자리에 떴다
    pub top: f32,
}
...
mod tests {
    ...
    #[test]
    fn 메뉴_줄은_자기_높이를_알린다() {
        // **하위 메뉴를 고른 줄 옆에 띄우는 규칙(2026-08-26)이 이 값에 걸려 있다** —
        // 종전에는 부모 메뉴 맨 위에 붙어 어느 줄을 펼쳤든 같은 자리에 떴다
        let ctx = egui::Context::default();
        let mut 위 = None;
        let mut 아래 = None;
        let _ = ctx.run_ui(egui::RawInput::default(), |ui| {
            위 = Some(menu_row_rich(ui, MenuRowIcon::None, "첫째", "", true, true));
            아래 = Some(menu_row_rich(ui, MenuRowIcon::None, "둘째", "", true, true));
        });
        let 위 = 위.expect("첫 줄");
        let 아래 = 아래.expect("둘째 줄");
        assert!(
            아래.top > 위.top,
            "아래 줄이 더 큰 y를 가져야 한다 (위 {}, 아래 {})",
            위.top,
            아래.top
        );
        // 두 줄 사이가 적어도 한 줄 높이만큼 벌어져 있다 — 같은 값을 두 번 돌려주면
        // 하위 메뉴가 여전히 한 자리에 몰린다
        assert!(아래.top - 위.top >= theme::MENU_ITEM_HEIGHT);
    }

    #[test]
    fn 메뉴_줄은_마우스가_얹힌_것을_알린다() {
        // **하위 메뉴를 마우스로 펼치는 규칙(2026-08-26)이 이 값에 걸려 있다** — 그리는
        // 쪽은 이 함수가 주는 `hovered`만 보고 펼칠지 정한다.
        //
        // **두 프레임이 필요하다**: egui는 **직전 프레임에 등록된 위젯 사각형**으로
        // hit-test하므로, 포인터를 옮긴 그 프레임에는 아직 잡히지 않는다
        // (`ui::titlebar`의 배지 시험이 같은 이유로 프레임을 나눈다)
        let ctx = egui::Context::default();

        // 1프레임 — 줄을 그려 자리를 등록한다. 포인터가 없으니 얹힘이 아니다
        let mut 자리 = egui::Pos2::ZERO;
        let mut 첫판 = None;
        let _ = ctx.run_ui(egui::RawInput::default(), |ui| {
            자리 = ui.cursor().min;
            첫판 = Some(menu_row_rich(ui, MenuRowIcon::None, "열기", "", true, true));
        });
        let 첫판 = 첫판.expect("첫 프레임이 줄을 그려야 한다");
        assert!(!첫판.hovered, "포인터가 없는데 얹혔다고 나왔다");
        assert!(!첫판.clicked);

        // 2프레임 — 그 줄 한가운데로 포인터를 옮긴다
        let 가운데 = 자리 + egui::vec2(20.0, theme::MENU_ITEM_HEIGHT / 2.0);
        let mut input = egui::RawInput::default();
        input.events.push(egui::Event::PointerMoved(가운데));
        let mut 둘째판 = None;
        let _ = ctx.run_ui(input, |ui| {
            둘째판 = Some(menu_row_rich(ui, MenuRowIcon::None, "열기", "", true, true));
        });
        let 둘째판 = 둘째판.expect("둘째 프레임이 줄을 그려야 한다");
        assert!(
            둘째판.hovered,
            "마우스를 올렸는데 얹힘이 잡히지 않았다 — 이 값이 거짓이면 하위 메뉴가 펼쳐지지 않는다"
        );
        assert!(!둘째판.clicked, "누르지 않았는데 눌렸다고 나왔다");
    }
}
