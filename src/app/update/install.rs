...
///
/// **설치 직후 첫 실행에서는 실패할 수 있다** — 설치 프로그램이 아직 이 폴더 안에서
/// 돌고 있으면 그 파일이 잠겨 있다(설치기가 앱을 띄우고 나서 자기 종료를 마치기까지의
/// 짧은 구간). 실패를 되돌리지 않고 넘기는 이유는 **다음 실행이 같은 자리에서 다시
/// 치우기** 때문이다 — 여기서 기다리면 그만큼 시작이 늦어지는데, 남은 것은 다음 번에
/// 확실히 사라지므로 값을 치를 이유가 없다
...
/// `progress`는 조각을 받을 때마다 `(받은 누적 바이트, 전체 크기)`로 불린다
pub fn download_and_verify(
    ...
    http::download_to_file(&info.asset_url, &dest, progress)?;
